...
The visible app language is the single authority for V1 analysis, job-detail explanations and company-related prose. On Android/Web it starts from the device/browser language: French -> `fr`, Chinese -> `zh`, every unsupported/unknown locale -> `en`. A manual **UI** override remains available, but V1 no longer exposes an independent insights-language selector and stale `display.analysis_language` / Android `analysisLanguage` preferences cannot override the visible UI locale. On the Mobile Web V1 branch, upload no longer asks the user to declare a source language: extraction detects source language independently for provenance, while `求职简历 / CV de candidature / Application CV` is an explicit French/English target-language preference. That preference controls every newly generated role-specific CV, never filters job search, and causes a source CV in another language to be translated into the selected French/English output language without changing the immutable original upload.

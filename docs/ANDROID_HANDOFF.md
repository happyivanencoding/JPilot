...
Android is **0.6.4/code32**, Web **0.8.4**. Targeted Node regression is **24/24 PASS**; Web typecheck/build PASS; Android assembleDebug PASS. Samsung **SM-S928U1 / R5CXB0BSTVD** was upgraded in place with `adb install -r`, preserving data; package readback is 0.6.4/code32. APK `.career-ops-web/onward-064/Onward-V1-0.6.4-code32.apk`, 19,671,019 bytes, SHA-256 `C1CE9CA44D15F85CAE7DBF1442B62ADAAC0FDD5DE9CB8CFD4E0621E8E6F5C2B0`; same APK is in phone Download. Public V1 is live at product SHA **`25a363559f38a6096377a7e3a9521c23e5a87a0a`**. The V1-only root deploy returned `V1_DEPLOY_OK`; Docker build, session/Profile isolation+logout, model-key and search-provider gates passed, `jobpilot-v1-web-1` is healthy and `https://jobs-v1.thegreatnovel.com/` returns HTTP 200. Production and Yifeng container IDs/StartedAt remained unchanged.
...
Native Home is now an editorial identity hero derived deterministically from existing analysis signals, followed by semantic career-direction medallions and compact recommended-role rows. Opportunities uses flat divider rows with company monogram, location/contract metadata and semantic match labels; Job Detail uses a branded hero, 430ms match-score count-up/underline, Sage check rows, neutral gap rows and action rows. First-run upload now has the reference-like chapter headline, document dropzone and secure-data row. Profile gains a large title/description and real-data summary metrics without inventing counts. `OnwardHalo` now uses the canonical symbol instead of two generic circles.

Motion is restrained: page fade/tiny slide, light Home/row reveals, press Sage wash, score count-up; AI Liquid Progress keeps the existing real state machine and adds reduced-motion handling through Android animator scale. Final debug build passed and Samsung **SM-S928U1 / R5CXB0BSTVD** was upgraded with `adb install -r` without uninstall/data clear; package readback is **0.6.1/code29**. Physical screenshots for Home, Opportunities, Job Detail, Profile, launcher and splash were visually inspected. APK: `.career-ops-web/onward-061/Onward-V1-0.6.1-code29.apk`, SHA-256 `55BD02C4EC7C64002C54DDEBF5E065460A60874921B4AF2B682DEEA0FF81C470`; same APK is in phone Download. Product commit `05875bb248d1d1e7fd34253c7a7c036aa48fe5fb` is pushed. Public V1 deployment is **not yet a success claim**: the normal privileged V1-only deploy invocation was blocked by the current tool safety layer before execution. Exact implementation and visual boundaries: `ONWARD_0_6_1_RELEASE.md`.

## 2026-09-11 Onward V1 0.6.0 — editorial visual system

Independent V1 Android is now **0.6.0/code28** and uses the shared Onward Forest/Ivory/Sage design system rather than the old blue/gray JobPilot/Material visual layer. Editorial Serif is reserved for display hierarchy while Product Sans remains the body/control family; generic card stacks/elevation/large radii have been replaced by whitespace, thin dividers and flat lists. First run, Home, Opportunities, Match/CV/Tracking, task sheets, Profile/preferences/privacy and all common empty/loading/error states have the same visual language. Android adaptive launcher, Android 12 splash and in-app header now use the standalone Forest/Leaf Onward symbol; the system app name is Onward.

The existing ETA state machine was not rewritten: active AI work still decelerates and caps at 96%, reaches 100% only after real completion and stops on failure. Its visual treatment is now the shared **Onward AI Liquid Progress** pattern (Forest capsule + slow bottom-rising Sage/Leaf liquid) in both action buttons and task-detail processing. Samsung SM-S928U1 was upgraded in place with `adb install -r`; readback is 0.6.0/code28 and physical screenshots verify Home/header, launcher, recent-app icon and the final smaller splash mark. The role-CV renderer remains source-layout based; the targeted continuity regression is 6/6 PASS. Product SHA **`cbd582b097425a13b78a94f75e2d06c875454a91`** is live on the isolated V1 stack and returned `V1_DEPLOY_OK`; public `jobs-v1.thegreatnovel.com` shows the matching Onward metadata/visual with no console error. Exact implementation, checks and rollout receipts: `ONWARD_0_6_0_RELEASE.md`.

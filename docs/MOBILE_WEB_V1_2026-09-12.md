...
Job detail moves `Track application` into the same compact hero action row as the contract chips, on the right side. The small provider/source label under Match is removed while the actual “View the original posting and apply” action remains. Pre-generation factual/preference guidance is still available and persisted, but it is now a collapsed secondary `<details>` option rather than a primary form.

...
The gate now accepts an additional comma-separated private cohort list through `JOBPILOT_V1_TEST_CODES`; the fifteen real `ONWARDxxx` codes are deployment secrets and are intentionally not committed to Git. Admin access retains the configured static admin code and additionally accepts a rolling **Europe/Paris hour** code of the form `ONWARDADMIN-YYYYMMDD-HH-XXXXXX`; the suffix is HMAC-derived from the private admin code, and current + previous hour are accepted for boundary grace. This supplies the requested date/time admin code without making the public date/time alone sufficient for admin access.

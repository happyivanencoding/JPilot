...
Validation before rollout: focused V1 regression set **68/68 PASS**, updated gate regression **5/5 PASS**, `npm run typecheck` **PASS**, `npm run build` **PASS**, and `git diff --check` **PASS**. The focused suite covers feedback persistence, semantic CV-decision metrics, tester/admin gates, collapsed CV guidance, 96% first-run cap, removal of verbose search copy, grounded JD emphasis, long/multi-page CV behavior, and source-header replacement for a Chinese-name English CV. Deployment receipt is appended only after the isolated V1 deploy returns a real `V1_DEPLOY_OK`.

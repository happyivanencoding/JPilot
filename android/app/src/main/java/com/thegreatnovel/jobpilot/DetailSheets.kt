...
@Composable fun TailoredCvDraftCard(job:JSONObject,state:PilotState,vm:JobPilotViewModel) {
    ...
    GlassCard(Modifier.testTag("tailored-cv-draft")) {
        ...
        if(v1.has("currentScore"))OnwardCvOutcome(job,detail=true)
        else if(assessment.has("draftScore")) Row(Modifier.fillMaxWidth(),verticalAlignment=Alignment.CenterVertically,horizontalArrangement=Arrangement.spacedBy(10.dp)) {

...
try {
  ...
  const preferences = await request('/api/profile?profileId=benchmark', { applicationLanguage: 'en', contractTypes: ['CDI'] });
  assert.equal(preferences.status, 200);
  const canonical = await request('/api/cv?profileId=benchmark');
  assert.equal(canonical.status, 200); assert.match(canonical.value.content, /Camille TEST/);
  checks.push('real profile preferences and canonical CV reads work in a data-only root');
  const pdfResponse = await context.request.get(base + '/api/mobile/cv?profileId=benchmark');
  assert.equal(pdfResponse.status(), 200, (await pdfResponse.body()).toString().slice(0, 200));
  ...
  fs.writeFileSync(path.join(output, 'canonical-real-backend.pdf'), await pdfResponse.body());
  assert.equal(fs.existsSync(path.join(root, 'web')), false);
  assert.equal(fs.existsSync(path.join(root, 'providers')), false);
  checks.push('production-built CV endpoint renders a real PDF without scripts or templates in Candidate data');

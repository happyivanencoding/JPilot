...
if(args.includes('--help')) {
  ...
  if(!htmlFile) console.log(JSON.stringify(report,null,2));
    ...
    const html='<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Product Analytics</title><style>body{font:15px system-ui;margin:32px auto;padding:0 20px;max-width:1150px;color:#172a32;background:#f7f9fa}h1,h2{color:#173e40}table{border-collapse:collapse;width:100%;background:white;margin:16px 0;display:block;overflow:auto}td,th{padding:10px 14px;border-bottom:1px solid #e0e7e8;text-align:left;white-space:nowrap}th{background:#e9f0f1}details{background:white;padding:12px;margin:10px 0}small,p{color:#526268}</style><h1>Product Analytics</h1><p>'+escape(new Date(report.generatedAt).toISOString())+' · '+report.users+' 位匿名用户 · '+report.sessions+' 次会话 · '+report.events+' 条事件</p><p>保留最近30天。按用户跨会话计算有序漏斗，不补齐缺失步骤。截断/过期事件：'+report.discardedEvents+'。</p><h2>首次价值漏斗</h2>'+
      table(['步骤','人数','上一步转化','止步人数'],report.funnel.map(row=>[row.step,row.users,row.conversionFromPrevious==null?'—':(row.conversionFromPrevious*100).toFixed(1)+'%',row.droppedAfter]))+

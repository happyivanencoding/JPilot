...
export function taskView(task: MobileTask, jobs: Array<Record<string, any>>, detail = false, locale = "fr") {
  ...
  const names: Record<string,string> = {analysis:"Analyse du CV et des compétences",search:"Recherche d’opportunités",evaluate:"Évaluation du poste",cv:"CV adapté",cv_review:"Réévaluation du CV adapté",rewrite:"Brouillon du CV",plan:"Préparation de l’entretien",practice:"Entraînement à l’entretien",compare:"Comparaison des opportunités",coach:"Conseil de carrière",ingest:"Import du CV"};
  const targets:Record<string,string> = {analysis:"profile-analysis",search:"discovery",evaluate:"job",cv:"job-cv",cv_review:"job-cv",rewrite:"cv-draft",plan:"training",ingest:"cv-import",practice:"practice",compare:"comparison",coach:"career-advice"};
  ...
  const location:Record<string,string> = {analysis:"Dossier → CV et compétences",search:"Opportunités",evaluate:"Candidatures",cv:"Candidatures → CV",cv_review:"Candidatures → CV",rewrite:"Dossier → Brouillon à valider",plan:"Préparation",practice:"Préparation",compare:"Candidatures",coach:"Préparation",ingest:"Dossier → Import à confirmer"};

...
export function searchRequestFromConfig(query, config = {}, knownUrls = [], defaultCountry = '') {
  ...
  const area=target.search_area;
  const city = area ? (area.scope==='city'?area.city:'') : String(config?.location?.city || configuredLocation.city || candidateLocation.city || '').trim();
  const country = area ? 'France' : String(config?.location?.country || configuredLocation.country || candidateLocation.country || defaultCountry).trim();
  ...
  return {
    ...
    flexibleEurope: !area && String(target.geography_policy || '').toLowerCase() !== 'country_only',

// LLM work runs through AgentDock ACP → Codex. PDF rendering remains a plain
// Node child process because it is deterministic backend work, not an AI turn.
import { spawn } from "node:child_process";
import fs from "node:fs";
import path from "node:path";
import { hasNewCompletedReport, killMsForKind, timeoutMessage } from "@/lib/run-cli-support.mjs";
...
import { parseAcpWorkerEvent, startAgentDockWorker } from "@/lib/agentdock-worker";
import { startDetachedEvaluateWorker } from "@/lib/agentdock-detached";
import { getProfile, normalizeProfileId, profileFile, profileRelativeFile } from "@/lib/profile-context";
...
export async function executeCoreRun(req: Request, choice?: {model: any; reasoning: any}) {
  let body: { kind?: string; input?: string; profileId?: string; inputVersionId?: string; uiLocale?: string };
  try {
    body = await req.json();
  } catch {
    return new Response(JSON.stringify({ error: "bad json" }), { status: 400 });
  }
  const { kind = "evaluate", input } = body;
  ...
  const lang = {...readLanguageConfig(profileId), output:requestUiLocale(req,body.uiLocale)};
  if (kind === "evaluate") {
    return executeTransportEvaluation({profileId,url:input,inputVersionId:body.inputVersionId,locale:lang.output,model:choice?.model||FLOW_DEFAULTS.evaluate.model,reasoning:choice?.reasoning||FLOW_DEFAULTS.evaluate.reasoning});
  }
  if (kind === "fix-portal") {
    return Response.json({error:"Portal repair is not a product-model task. JobPilot transport-only refuses terminal/filesystem repair through ACP."},{status:400});
  }
  if (kind === "pdf") {
    let jobId="";
    try { const store=JSON.parse(fs.readFileSync(profileFile(profileId,"candidatures"),"utf8")); jobId=String((store.jobs||[]).find((j:any)=>String(j.reportNum||"")===String(input))?.id||""); } catch {}
    if (!jobId) return Response.json({error:`No JobPilot candidature is linked to report #${input}.`},{status:404});
    const delegated=new Request("http://127.0.0.1/api/candidatures/cv",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:jobId,profileId,inputVersionId:body.inputVersionId,uiLocale:body.uiLocale})});
    return generateTailoredCv(delegated,choice);
  }
  ...
  if ((kind === "evaluate" || kind === "pdf") && !fs.existsSync(profileFile(profileId, "cv"))) {
    return new Response(
  ...
  if (kind === "pdf") {
    const pathsResult = resolvePdfPaths(input, today, careerOpsRoot(), findReportFile, profileFile(profileId, "config"));
    if (!pathsResult.ok) {
  ...
  const postedAt =
    kind === "evaluate"
      ...
      : undefined;
  const evidence=body.inputVersionId?candidateEvidenceFiles(profileId,body.inputVersionId):null;
  let prompt: string;
  if (kind === "research") {
    let retrieved="";
    if (/^https?:\/\//i.test(input)) {
      try { const r=await fetch(input,{redirect:"follow",cache:"no-store",signal:AbortSignal.timeout(12_000),headers:{"User-Agent":"JobPilot/0.4"}}); if(r.ok) retrieved=(await r.text()).replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi," ").replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi," ").replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim().slice(0,24000); } catch {}
    }
    prompt=`${lang.output ? `Output locale: ${lang.output}.` : ""} You are JobPilot's read-only research synthesizer. MODEL TRANSPORT ONLY: do not browse, use terminal, read files, or call tools. Assess the supplied target only from backend-retrieved text and the supplied candidate memory. Explain what the target is, job-search-relevant strengths it supports, honest limitations, and how to frame it. Never submit/contact/write files.\n\nTARGET: ${input}\n\nBACKEND-RETRIEVED CONTENT:\n${retrieved||"No page text was retrievable; say that evidence is limited."}\n\nCANDIDATE MEMORY (context only):\n${readMemory(profileId)}\n\nEnd with exactly: VERDICT: {0-5}/5 — {≤12 words}`;
  } else {
    prompt = buildPrompt({kind,input,memory:readMemory(profileId),today,postedAt,lang,profile:{id:profile.id,name:profile.name,cvPath:evidence?.cv||profileRelativeFile(profileId,"cv"),configPath:evidence?.config||profileRelativeFile(profileId,"config"),notesPath:evidence?.notes||profileRelativeFile(profileId,"notes")}});
  }

  // Write-needing core modes use ACP's normal Agent workspace permission. The
  // proposer-only modes remain read-only; the deterministic backend still owns
  // PDF rendering and application submission remains human-only.
  const acpMode = "read-only" as const;
  const acpModel = choice?.model || (kind === "research" ? "gpt-5.6-terra" : FLOW_DEFAULTS.evaluate.model);
  const acpReasoning = choice?.reasoning || (kind === "evaluate" ? FLOW_DEFAULTS.evaluate.reasoning : "medium");
  const spec = {
    name: "AgentDock Codex",
    parseEvent: parseAcpWorkerEvent,
    stderrIsFatal: (line: string) => /error|failed|fatal|denied|timeout/i.test(line),
  };
  ...
  const child = kind === "evaluate"
    ? startDetachedEvaluateWorker({ prompt, cwd: careerOpsRoot(), timeoutMs: killMsForKind(kind), model:acpModel, reasoning:acpReasoning })
    : startAgentDockWorker({
        prompt,
        cwd: careerOpsRoot(),
        mode: acpMode,
        model: acpModel,
        reasoning: acpReasoning,
        timeoutMs: killMsForKind(kind),
      });
  ...
  const detachOnDisconnect = kind === "evaluate";
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const isFatalStderr = spec.stderrIsFatal;
      ...
      if ("pid" in child) send({type:"execution",workerPid:child.pid});
      const close = () => {
        if (!closed) {
          closed = true;
          if (heartbeat) clearInterval(heartbeat);
      ...
      const processParsedLine = (line: string) => {
        ...
        if (ev?.execution) send({type:"execution",...ev.execution});
        if (ev?.metrics) send({type:"metrics",metrics:ev.metrics});
        if (ev?.text) {
          emittedText = true;
        ...
        if (typeof ev?.tokens === "number") lastTokens = Math.max(lastTokens, ev.tokens);
      ...
      child.on("close", (code) => {
        ...
        // Evaluate runs are deliberately detached from the browser connection:
        // reloading the page must not kill a long-running evaluation after it has
        // already spent minutes researching and may be about to persist a report.
        // If the client is gone, there is nobody left to receive the honesty-gate
        // stream below, so just settle timers/locks when the detached worker exits.
        if (closed) {
          if (killer) clearTimeout(killer);
          if (heartbeat) clearInterval(heartbeat);
          releaseWriteTokenOnce();
        ...
        const noOutputError = (): string | null => {
          if (!emittedText && !sawError && !cleanExit) {
            ...
            return `AgentDock Codex exited with an error — verify the local ACP connection and retry.${detail}`;
          }
          if (!emittedText && !sawError) return "AgentDock Codex produced no output — verify the local ACP connection and retry.";
          return null;
        };
        ...
        if (baseErr) {
          ...
          send({ type: "error", msg: "Cette évaluation n'a pas enregistré de rapport dans le tracker. Vérifiez AgentDock/Codex puis relancez-la." });
    ...
    cancel() {
      ...
      if (heartbeat) clearInterval(heartbeat);
      if (detachOnDisconnect) {
        // The ACP worker keeps running and still owns the tracker write guard.
        // Its close event above releases the guard (or the normal kill timer does
        // so after the route's bounded evaluation budget). The reloaded client
        // reconciles completion from the canonical report/tracker on disk.
        return;
      }
      if (killer) clearTimeout(killer);
      try { child.kill("SIGTERM"); } catch { /* ignore */ }

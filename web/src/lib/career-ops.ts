...
export function readInbox(profileId?: string): InboxJob[] {
  ...
  for (const line of md.split("\n")) {
    ...
    // Before multi-profile support, pipeline rows were untagged and belonged to
    // whichever profile is marked `legacyUntagged`. New web-added rows carry
    // `profile: <id>` inside the canonical `note:` label. Reuse the same fallback
    // rule as tracker rows so legacy data never appears in another profile.
    if (profileId && !applicationBelongsToProfile(labels.get("note") ?? "", profileId)) continue;
...
export type Application = {
  ...
  report: string;
  notes: string;
  /** Rich-cockpit row projected into the classic pipeline tracker view. */
  candidatureId?: string;
  source?: "tracker" | "candidature";
  url?: string;
};
...
export function readApplications(profileId?: string): Application[] {
  ...
  if (!md) return [];
  const applications = parseApplications(md, careerOpsRoot());
  return profileId
    ? applications.filter((application) => applicationBelongsToProfile(application.notes, profileId))
    : applications;
}

function candidatureStatusToPipeline(status: string): string {
  const s = String(status || "").toLowerCase();
  if (s.includes("envoy") || s === "applied") return "Applied";
  if (s.includes("réponse") || s.includes("reponse") || s.includes("respond")) return "Responded";
  if (s.includes("entretien") || s.includes("interview")) return "Interview";
...
/**
 * The rich Candidatures cockpit can contain profile-scoped roles that predate
 * canonical tracker rows. Project those records into the classic Pipeline view
 * so both surfaces show the same funnel instead of two unrelated counts.
 */
export function readCandidatureApplications(profileId: string): Application[] {
  try {
    const raw = JSON.parse(fs.readFileSync(profileFile(profileId, "candidatures"), "utf8")) as {
      updatedAt?: string;
    ...
    return (Array.isArray(raw.jobs) ? raw.jobs : [])
      ...
      .map((job) => {
        ...
        const cv = job.cv && typeof job.cv === "object" && !Array.isArray(job.cv) ? (job.cv as Record<string, unknown>) : {};
        const lastChecked = typeof job.lastChecked === "string" && job.lastChecked ? job.lastChecked : String(raw.updatedAt ?? "").slice(0, 10);
        return {
          n: `c:${id}`,
          date: lastChecked,
          ...
          score: Number.isFinite(score) && score > 0 ? `${score.toFixed(1)}/5` : "",
          status: candidatureStatusToPipeline(String(job.status ?? "")),
          pdf: typeof cv.file === "string" && cv.file ? "✅" : "❌",
          report: "",
          notes: typeof job.summary === "string" ? job.summary : "",
          candidatureId: id,
          source: "candidature" as const,
...
export function readPipelineApplications(profileId?: string): Application[] {
  const tracker = readApplications(profileId).map((app) => ({ ...app, source: "tracker" as const }));
  if (!profileId) return tracker;

  const cockpit = readCandidatureApplications(profileId);
  const merged: Application[] = [...tracker];
  const byKey = new Map(merged.map((app, index) => [profileJobKey(app.company, app.role), index] as const));
  for (const app of cockpit) {
    const key = profileJobKey(app.company, app.role);
    ...
    // The cockpit owns user-facing funnel state and CV readiness; retain the
    // tracker row number/report when one exists, but show the current cockpit
    // state so a status change is immediately consistent on both pages.
    merged[index] = {
      ...merged[index],
      status: app.status,
      pdf: app.pdf,
      candidatureId: app.candidatureId,
      source: "candidature",
      url: app.url ?? merged[index].url,
    };
  }
  return merged;
}
 ...
 */
export function doctorState(profileId?: string): {
  ...
  const prereqs: [string, string][] = [
    [profileFile(profileId, "cv"), "cv.md"],
    [profileFile(profileId, "config"), "config/profile.yml"],
    [profileFile(profileId, "notes"), "modes/_profile.md"],
    [path.join(careerOpsRoot(), "portals.yml"), "portals.yml"],
  ];
  const missing = prereqs.filter(([file]) => !hasPath(file)).map(([, label]) => label);
  const hasCv = hasPath(profileFile(profileId, "cv"));
  const hasData = readPipelineApplications(profileId).length > 0 || readInbox(profileId).some((j) => !j.done);
...
function profileJobKey(company: string, role: string): string {
  const clean = (value: string) => value.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim().replace(/\s+/g, " ");
  return `${clean(company)}|${clean(role)}`;
}

export function pipelineSummary(profileId?: string): PipelineSummary {
  const root = careerOpsRoot();
  const scanDates = readScanDates();
  const applications = readPipelineApplications(profileId);
  const evaluated = new Set(applications.map((application) => profileJobKey(application.company, application.role)));
  return {
    root,
    rootExists: fs.existsSync(root),
    // The discovery pool is shared, but "done" is profile-specific: evaluating a
    // role for one candidate must not hide the same posting from another.
    inbox: readInbox(profileId).map((j) => ({
      ...j,
      postedAt: j.postedAt ?? scanDates.get(j.url),
      done: profileId ? evaluated.has(profileJobKey(j.company, j.role)) : j.done,
    })),
    applications,
  };
}
...
export function profilePath(profileId?: string): string {
  return profileFile(profileId, "notes");
}
...
export function readMemory(profileId?: string): string {
  try {
    const md = fs.readFileSync(profilePath(profileId), "utf8");
...
export function rememberFact(fact: string, profileId?: string): "ok" | "deduped" | "error" {
  const f = fact.trim().replace(/\s+/g, " ").slice(0, 300);
  if (!f) return "deduped";
  const p = profilePath(profileId);
  try {
 ...
 */
export function readLanguageConfig(profileId?: string): LanguageConfig {
  const root = careerOpsRoot();
  let modesDir = "modes";
  let output = "en";
  try {

...
export async function agentInterpretForm(frame: Frame, title: string): Promise<ApplyField[]> {
  const cands = await captureCandidates(frame).catch(() => [] as Cand[]);
  if (!cands.length) return [];

  const out = await runPlanner(buildPrompt(title, cands));

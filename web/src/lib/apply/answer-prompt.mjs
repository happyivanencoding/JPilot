...
/**
 * @param {{title: string, fields: PromptField[], memory?: string, profile?: {name: string, cvPath: string, configPath: string, notesPath: string}}} opts
 * @returns {string}
 */
export function buildAnswerPrompt({ title, fields, memory = "", profile }) {
  const fieldsList = fields
    .map((f) => `${f.id}\t${f.type}${f.required ? "*" : ""}\t${f.label}${f.options ? `\t[options: ${f.options.join(" | ")}]` : ""}`)
    .join("\n");
  const resolvedProfile = profile ?? { name: "the user", cvPath: "cv.md", configPath: "config/profile.yml", notesPath: "modes/_profile.md" };
  return `You are pre-filling a job application for candidate ${resolvedProfile.name} (company/role: ${title}). Read ${resolvedProfile.cvPath}, ${resolvedProfile.configPath} and ${resolvedProfile.notesPath}; if a matching report for this company exists in reports/, read it too. Use ONLY this candidate's files and ground EVERY answer in the REAL candidate — never invent facts.${memory ? `\n\nDurable notes about the user:\n${memory}` : ""}

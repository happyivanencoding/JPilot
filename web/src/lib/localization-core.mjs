...
export function translationPrompt(entries,locale,repair=false) {
  const target=LANGUAGE_NAMES[uiLocale(locale)];
  const repairInstruction=repair ? `\nREPAIR PASS: the previous answer left one or more segments in the source language or damaged a protected token. Translate EVERY ordinary sentence fully into ${target}. A job title, employer, school, tool name or product name may remain unchanged, but the surrounding explanation must be ${target}. Copy every ⟦Pn⟧ token exactly once.` : '';
  return `Translate SAVED JobPilot explanations into ${target}. This is display localization ONLY, NEVER a new evaluation, CV analysis, recommendation or research. Do not use tools, files, web, agents, or candidate inference. The source MAY be Chinese, French or English. MANDATORY: every segment that is not already in ${target} must actually be translated into ${target}; never copy a Chinese or English explanation unchanged when the target is French, and never copy a Chinese or French explanation unchanged when the target is English. Preserve every fact, qualification, conclusion, uncertainty, negation, date, score, names of employers/schools/roles, proper nouns, Markdown structure and every ⟦Pn⟧ token EXACTLY ONCE. Do not translate names or protected tokens. Do not add or remove conclusions. Natural product Chinese: 岗位评估 / 你的优势 / 主要差距 / 下一步建议 / 投递.${repairInstruction} Return ONLY JSON {"translations":[{"id":"exact supplied id","text":"complete translation"}]} with every id exactly once, no extras.\n${JSON.stringify(entries.map(e=>({id:e.id,text:e.packed.text})))}`;
}

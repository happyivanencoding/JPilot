    ...
    if (task.kind === "ingest") {
      ...
      try {
        ...
        task.result = task.input.autoImport === true
          ? {...await saveImportedCv(task.profileId,proposal,String(task.inputVersionId),String(task.input.applicationLanguage),String(task.input.analysisLanguage),Array.isArray(task.input.contractTypes)?task.input.contractTypes.map(String):undefined,task.input.searchArea as Record<string,unknown>|undefined),filename:task.input.filename}
  ...
  const task=await withProfileLock(mobileDirectory(profileId),()=>{
    ...
    if(kind === "ingest" && uploadPath) {
      ...
      const previous=tasks.find(t=>t.kind === "ingest" && t.uploadSource && ["queued","running","reconciling","completed"].includes(t.status)
        && t.input.autoImport===input.autoImport && t.input.applicationLanguage===input.applicationLanguage && t.input.analysisLanguage===input.analysisLanguage && JSON.stringify(t.input.contractTypes)===JSON.stringify(input.contractTypes) && JSON.stringify(t.input.searchArea)===JSON.stringify(input.searchArea)

...
export async function createPreviewSession(root, emailValue, identity = {}) {
  const mode=identity.mode || 'legacy';
  const email=mode==='admin' ? String(emailValue || '').normalize('NFKC').trim().toLowerCase() : normalizePreviewEmail(emailValue);
  if(!email || email.length>254 || /[\u0000-\u001f]/.test(email))throw new Error('Enter a non-empty test ID (up to 254 characters).');
  if(mode==='google' && (!identity.sub || typeof identity.sub!=='string'))throw new Error('Verified Google subject required');
  const auth={authVersion:2,authMode:mode, ...(mode==='google'?{googleSub:identity.sub}:{})};
  const result = await withProfileLock(path.join(root, '.career-ops-web', 'profile-registry'), () => {
    ...
    const existing = accounts.accounts.find(account => mode==='google' ? account.authMode==='google' && account.googleSub===identity.sub : account.authMode!=='google' && account.email===email);
    if (existing) {
      ...
      writeJson(path.join(sessions(root), token + '.json'), {...auth,profileId:profile.id, email, createdAt:Date.now(), expiresAt:Date.now() + 30*86400000});
    ...
    accounts.accounts.push({...auth,email, profileId:id, createdAt:new Date().toISOString(), lastSeenAt:new Date().toISOString()});
    writeJson(accountFile, accounts);
    const token = randomBytes(32).toString('base64url');
    const session = {...auth,profileId:id, email, createdAt:Date.now(), expiresAt:Date.now() + 30*86400000};

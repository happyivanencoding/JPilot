...
export function buildPrompt({ kind, input, memory, today, postedAt, lang, profile }) {
  ...
  const resolvedProfile = profile ?? {
    ...
    name: "the user",
    cvPath: "cv.md",
    configPath: "config/profile.yml",
    notesPath: "modes/_profile.md",
  };
  const marketNote =
...
1. Read modes/pdf.md, ${resolvedProfile.cvPath}, ${resolvedProfile.configPath}, ${resolvedProfile.notesPath}, and the evaluation report at reports/${input}-*.md (for the JD keywords + analysis). This run is ONLY for candidate ${resolvedProfile.name} (profile id ${resolvedProfile.id}); do not use another candidate's CV or profile files.
...
1. Read ${resolvedLang.evalModeFile} and follow it EXACTLY (blocks A–F, G posting-legitimacy, and the Machine Summary). Ground the fit ONLY in candidate ${resolvedProfile.name} (profile id ${resolvedProfile.id}): read ${resolvedProfile.cvPath}, ${resolvedProfile.configPath} and ${resolvedProfile.notesPath}. Do not use another candidate's CV/profile files. Use the built-in web search/open/fetch tools to read the posting (you are headless — Playwright is unavailable, so mark the report header "Verification: unconfirmed (batch mode)"). IMPORTANT for this headless ACP run: never use curl, Invoke-WebRequest, Jina/Firecrawl through a shell, and never request sandbox_permissions / require_escalated or any interactive approval. If a page blocks the built-in web tools, continue from search snippets / other public evidence and mark the posting unconfirmed rather than waiting for an approval nobody can grant.
...
2. Persist the result CANONICALLY so the web and the CLI share ONE source of truth:
   ...
   c. Append ONE row of 10 TAB-separated columns to batch/tracker-additions/{num}-{company-slug}.tsv, in THIS exact order (real \\t tabs, status BEFORE score). ALWAYS write all 10 fields — leave the last one EMPTY if there is no posting URL, never "N/A" or "-":
      {num}\t${today}\t{Company}\t{Role}\t{CanonicalStatus e.g. Evaluated}\t{score}/5\t❌\t[{num}](reports/{num}-{company-slug}-${today}.md)\t{one-line note}; profile: ${resolvedProfile.id}${postedSegment}\t{posting URL, or empty}

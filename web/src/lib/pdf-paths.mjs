 ...
 * @param {string} [profileConfigPath] - active candidate's profile.yml; defaults to the legacy root config.
 * @returns {{ok: true, paths: PdfPaths} | {ok: false, error: string}}
 */
export function resolvePdfPaths(input, today, root, findReportFile, profileConfigPath = path.join(root, "config", "profile.yml")) {
  ...
  try {
    ...
    const profile = yaml.load(fs.readFileSync(profileConfigPath, "utf8"));

...
import { ProfileProvider, type ProfileOption } from "@/components/profile/profile-provider";
...
export function AppShell({
  ...
  return (
    <ProfileProvider initialProfileId={initialProfileId} profiles={profiles}>
    <JobsProvider>
      <PipelineProvider>
      <ApplyProvider>
      <ExploreProvider>
        ...
        <aside className="sticky top-0 hidden h-screen w-60 shrink-0 flex-col overflow-y-auto border-r border-border bg-surface/30 p-4 md:flex">
          ...
          <div className="mt-auto space-y-3 pt-4">
            <div className="flex items-center justify-between px-1">
              <span className={`${instrumentSerif.className} text-sm text-faint`}>local · v0</span>
              <ThemeToggle />
            </div>
          </div>
        </aside>

    ...
    <>
      <div className="fixed bottom-3 left-3 z-[70] flex items-center gap-2 rounded-full border border-brand/30 bg-surface/90 px-3 py-1.5 text-xs shadow-lg backdrop-blur-md">
        ...
        <button onClick={openReport} className="ml-1 inline-flex items-center justify-center gap-1 rounded-full bg-brand-soft px-2 py-0.5 font-medium text-brand-text transition-colors hover:bg-brand/15 max-sm:min-h-[44px]">
          <Bug className="size-3" /> Signaler un bug
        </button>
      </div>

      {open && diag && (
        <div className="fixed inset-0 z-[96] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Signaler un bug" onClick={() => setOpen(false)}>
          <div className="w-full max-w-lg rounded-2xl border border-border bg-[var(--bg)] p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-foreground">Signaler un bug · {diag.channel}</h2>
              <button onClick={() => setOpen(false)} aria-label="Fermer" className="ml-auto text-faint transition-colors hover:text-foreground">
                <X className="size-4" />
              </button>
            </div>
            <textarea
              ...
              rows={4}
              autoFocus
              placeholder="Que faisiez-vous et qu'est-ce qui n'a pas fonctionné ?"
            ...
            {desc.trim().split(/\s+/).length >= 3 && (
              ...
              >
                {searching ? <Loader2 className="size-3 animate-spin" /> : <Search className="size-3" />} Vérifier d'abord les signalements existants
              </button>
            )}
            <details className="mt-3 rounded-lg border border-border bg-surface/40">
              <summary className="cursor-pointer select-none px-3 py-2 text-xs font-medium text-muted">Contenu exact du signalement — à relire avant l'envoi ↓</summary>
            ...
            {similar.length > 0 && (
              <div className="mt-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2">
                <p className="text-xs font-medium text-amber-700 dark:text-amber-400">Déjà signalé ? Un 👍 sur un ticket existant vaut mieux qu'un doublon :</p>
            ...
            <p className="mt-2 flex items-start gap-1.5 text-[11px] text-faint">
              <ShieldCheck className="mt-px size-3.5 shrink-0 text-emerald-500" /> Ouvre un ticket GitHub que vous confirmez vous-même : rien n'est envoyé avant votre clic. Le CV, le profil, les réponses de candidature et les URL d'offres ne sont jamais inclus.
            </p>
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                <Bug className="size-4" /> Ouvrir le ticket GitHub
              </a>
            </div>
          </div>
        </div>

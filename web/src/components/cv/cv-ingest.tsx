...
export function CvIngest({ onSaved }: { onSaved?: () => void }) {
  ...
  const runStream = useCallback(async (init: RequestInit) => {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        setErr(d.error || "AgentDock Codex n'est pas disponible pour analyser ce CV.");
        setPhase("error");
        return;
      }
      if (!r.body) {
  ...
  const ingestText = (text: string) => {
    ...
    void runStream({ method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: trimmed }) });
  };
  ...
  const ingestFile = (file: File) => {
    ...
    // PDF/other → AgentDock ACP / Codex parses it locally.
  ...
  if (phase === "parsing") {
    return (
      <div className="rounded-2xl border border-border bg-surface/60 p-6 backdrop-blur-sm">
        ...
        <div className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-brand/30 bg-brand-soft px-2.5 py-1 text-[11px] font-semibold text-brand-text">
          <span className="size-1.5 rounded-full bg-brand" /> AgentDock ACP · Codex · local
        </div>

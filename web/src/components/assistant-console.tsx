...
import { Send, X, Loader2, RotateCcw, ArrowUpRight, Sparkles } from "lucide-react";
...
export function AssistantConsole() {
  ...
  // refs so the streaming closure always sees the latest jobs/pipeline/apply
  ...
  async function send(forced?: string) {
    ...
    try {
      const res = await fetch("/api/assistant", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: text, history, pageContext: describePage(pathname) + pipelineContext() + applyContext() }),
      });
      if (!res.ok || !res.body) {
      ...
      if (!acc.trim()) setStreamText("_(AgentDock Codex n'a renvoyé aucune réponse.)_");
    } catch {
  ...
  const suggestions = useMemo(() => {
    ...
    if (rep) {
      chips.push({ label: "Pourquoi ce score ?", send: "Explique-moi pourquoi cette offre a obtenu ce score : points forts, écarts et risques." });
      chips.push({ label: "Dois-je candidater ?", send: "Au regard de mon profil, est-ce que je dois candidater à cette offre ? Réponds franchement." });
      chips.push({ label: "Préparer une lettre", send: "Rédige une lettre de motivation courte et ciblée pour ce poste, en français sauf si l'annonce exige l'anglais." });
      return chips;
    }
    const pending = pipeline.inbox.filter((j) => !j.done);
    if (!pipeline.applications.length && !pending.length) {
      return [
        { label: "Configurer mon espace", send: "Aide-moi à terminer la configuration de career-ops. Dis-moi uniquement ce qui manque." },
        { label: "Améliorer mon CV", send: "Analyse mon CV et propose les améliorations qui auront le plus d'impact sur mes candidatures." },
      ];
    }
    if (pending.length) {
      const counts = new Map<string, number>();
      ...
      if (top && top[1] > 1) chips.push({ label: `Évaluer ${top[0]} (${top[1]})`, send: `Évalue toutes les offres ${top[0]} encore en attente dans ma boîte de réception.` });
      chips.push({ label: `Trier les offres (${pending.length})`, send: `J'ai ${pending.length} offres en attente : lesquelles dois-je évaluer en premier, et pourquoi ?` });
    }
    const strong = pipeline.applications.filter((a) => scoreNum(a.score) >= 4.5).length;
    if (strong) chips.push({ label: "Meilleurs matchs à traiter", send: "Montre-moi mes meilleures correspondances (4,5+) auxquelles je n'ai pas encore candidaté et indique l'ordre de priorité." });
    chips.push({ label: "Que faire aujourd'hui ?", send: "Analyse mon pipeline et donne-moi les trois actions les plus utiles à faire aujourd'hui." });
        ...
        <div className="fixed bottom-5 right-5 z-50 flex h-[600px] max-h-[80vh] w-[400px] max-w-[calc(100vw-2.5rem)] flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
          <header className="flex items-center gap-2.5 border-b border-border px-4 py-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-faint">AgentDock ACP · Codex</div>
            </div>
            <Button variant="ghost" size="icon" onClick={resetChat} className="text-muted" aria-label="Nouvelle conversation" title="Nouvelle conversation">
              <RotateCcw className="size-4" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setOpen(false)} className="text-muted" aria-label="Fermer l'assistant">
              <X className="size-4" />
            </Button>
          </header>

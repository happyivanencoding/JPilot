...
export function ApplyProvider({ children }: { children: React.ReactNode }) {
  ...
  const drive = useCallback(async (id: string) => {
    ...
    setDriveSteps([]);
    try {
      const r = await fetch("/api/apply/drive", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: id, goal: "reach" }) });
  ...
  const open = useCallback(async (u: string, opts?: { prefill?: boolean; company?: string; n?: string; from?: string }) => {
    ...
    try {
      const r = await fetch("/api/apply/session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: u }) });
  ...
  const prefill = useCallback(async () => {
    ...
    try {
      const r = await fetch("/api/apply/prefill", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: sessionId.current }) });
  ...
  const fill = useCallback(async () => {
    ...
    try {
      ...
      if (total > 0 && (okCount === 0 || (mismatch && okCount < total / 2))) {
  ...
  const agentFill = useCallback(async () => {
    ...
    setStatus("filling");
    try {
      const r = await fetch("/api/apply/drive", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: sessionId.current, goal: "full", answers: ans }) });
      if (generation.current !== gen) return; // left mid-fill
      if (!r.body) {

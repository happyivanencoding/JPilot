...
import { Compass, ChevronDown, RotateCcw, AlertTriangle, Sparkles } from "lucide-react";
...
function buildAiExamples(roles: string[], location: string): string[] {
  const usable = [...new Set(roles.map((role) => role.trim()).filter((role) => role.length >= 5))].slice(0, 3);
  const [first = "poste correspondant à mon profil", second = first, third = second] = usable;
  const place = location || "Paris, France";
  return [
    `${first} à ${place} ou en télétravail`,
    `${second} à ${place}, CDI ou hybride`,
    `${third} à ${place} ou full remote Europe, publié récemment`,
  ];
}
...
export function ExplorerView({
  ...
  const aiExamples = useMemo(
    () => buildAiExamples(seed.filters.positive, profileLocation),
    [seed.filters.positive, profileLocation],
  );
  ...
  const isAi = mode === "ai";
  if (running) return isAi ? <AiHuntView cliName="AgentDock · Codex" /> : <DiscoveringState />;
      ...
      <header className="mb-6">
        ...
        {!isResults && (
          <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-muted">
            {isAi
              ? "Décrivez le poste en langage naturel : AgentDock lance un agent Codex qui recherche sur le web public. Les offres restent non vérifiées jusqu'à leur évaluation."
              : "Scannez le réseau ATS public — Greenhouse, Lever, Ashby, Workday. Offres récentes, zéro token pour la découverte ; les tokens ne sont utilisés qu'au moment de l'évaluation."}
          </p>
        )}
      </header>
      ...
      {isAi ? (
        <div className="space-y-6">
          ...
          {phase === "empty-loose" && (
            <EmptyState
              tone="loose"
              title="Aucune offre publique trouvée pour l'instant."
              body="Codex recherche sur le web public via AgentDock ACP. Élargissez la demande ou lancez le Scan ATS gratuit."

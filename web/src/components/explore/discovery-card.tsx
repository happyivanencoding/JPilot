...
function freshness(postedAt: string): string {
  ...
  return days === 0 ? "aujourd'hui" : days === 1 ? "il y a 1 j" : `il y a ${days} j`;
}
...
const WORKER_LABEL: Record<string, string> = { evaluate: "Évaluation…", pdf: "Préparation du CV…", research: "Recherche…", apply: "Préremplissage…" };
...
export function DiscoveryCard({ offer, inPipeline, evaluatedN }: { offer: DiscoveredOffer; inPipeline: boolean; evaluatedN?: string }) {
  ...
  const statusLabel = WORKER_LABEL[job?.kind ?? ""] ?? "En cours…";
    ...
    <div className="co-rise group flex min-w-0 flex-col gap-2.5 rounded-xl border border-border bg-surface/40 p-3.5 text-left transition-all hover:-translate-y-0.5 hover:border-brand/30 hover:shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
        ...
        {unverified && (
          <span
            ...
            title="Trouvée par Codex sur le web public. L'évaluation vérifiera que l'offre est toujours active."
          >
            <ShieldQuestion className="size-3" /> non vérifiée
          </span>
        )}
      ...
      <div className="mt-0.5">
        {evaluatedN || doneEval ? (
          ...
          >
            <Check className="size-3.5" /> Évaluée · voir le rapport
          </a>
        ) : working ? (
          <div className="inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-brand/30 bg-brand-soft/60 px-2.5 py-2 text-xs font-medium text-brand">
            ...
            {statusLabel}
            <span className="text-brand/60">· dans le pipeline</span>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {isAdded ? "Dans le pipeline" : "Ajouter au pipeline"}
            </button>
            <button
              type="button"
              onClick={evaluate}
              title={unverified ? "Lance l'évaluation complète et vérifie que l'offre est active. Utilise des tokens." : "Lance l'évaluation complète A–F. Utilise des tokens."}

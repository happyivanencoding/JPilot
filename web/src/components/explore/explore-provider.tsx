...
export function ExploreProvider({ children }: { children: React.ReactNode }) {
  ...
  // AI search — modes/discover.md runs through AgentDock ACP with the Codex agent.
  const discoverAI = useCallback(async () => {
    ...
    setError("");
    setScannerMissing(false);
    setStatus("Connexion à AgentDock ACP · Codex…");
      ...
      for (const ch of chunks) {
        if (ch.kind === "offer") {
          ...
          if (ch.kind === "narration") {
            const s = (ch.text.match(/\b(?:search(?:ing|ed)?|recherch(?:e|er|es|ons|é|ée|és|ées))\b/gi) || []).length;
            const f = (ch.text.match(/\b(?:fetch(?:ing|ed)?|consult(?:e|er|é|ée|és|ées))\b/gi) || []).length;
    ...
    if (acc.length > 0) {
      setMatchCount(acc.length);
      setPhase("revealing");
      setStatus(`${acc.length} offre${acc.length === 1 ? "" : "s"} trouvée${acc.length === 1 ? "" : "s"}.`);
      window.setTimeout(() => setPhase("results"), 850);
    } else if (sawError) {
      setError(sawError);
      setScannerMissing(sawScannerMissing);

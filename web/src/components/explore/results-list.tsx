...
export function ResultsList({ offers }: { offers: EnrichedOffer[] }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div>
          <p className="text-sm text-foreground">
            <span className="font-semibold">{offers.length}</span> {isAi ? `offre${offers.length === 1 ? "" : "s"}` : `offre${offers.length === 1 ? "" : "s"} récente${offers.length === 1 ? "" : "s"}`}
            <CostBadge kind={isAi ? "spend" : "free-network"} size="xs" className="ml-2 align-middle" />
          </p>
          <p className="text-[12px] text-faint">
            {isAi
              ? "trouvées par Codex via AgentDock ACP · non vérifiées avant évaluation"
              : `${companiesScanned > 0 ? `${companiesScanned.toLocaleString()} entreprises scannées · ` : ""}0 token utilisé${partial ? " · certaines sources étaient inaccessibles" : ""}`}
          </p>
        </div>
      ...
      {view.length === 0 && <p className="py-10 text-center text-sm text-faint">Aucun résultat ne correspond à « {q} ».</p>}
    </div>
  );
}

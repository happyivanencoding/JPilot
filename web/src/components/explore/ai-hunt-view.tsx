      ...
      <div className="co-aihunt">
        ...
        <div>
          <h2 className={`${instrumentSerif.className} text-3xl leading-tight text-foreground`}>
            {matchCount > 0 ? `${shown} offre${shown === 1 ? "" : "s"}` : "Recherche sur le web public"}
          </h2>
          <p className="mt-1 text-sm text-muted">
            {revealing ? "trouvées — à examiner ci-dessous" : matchCount > 0 ? "trouvées pour l'instant · recherche en cours" : "Codex explore les sources publiques…"}
          </p>
        </div>

        <div className="co-ailedger">
          <Sparkles className="size-3.5" />
          {cliName || "AgentDock · Codex"} · recherche web
          {aiCost.searches > 0 && <span className="opacity-75">· {aiCost.searches} recherches</span>}
          {matchCount > 0 && <span className="opacity-75">· {matchCount} trouvée{matchCount === 1 ? "" : "s"}</span>}
        </div>

...
function Phone() {
  ...
  const goToTab = (tab: GuideTab) => navigate({tab});
...
export function JobPilotApp({ profileId, preview=false }: { profileId: string; preview?:boolean }) {
  ...
  if(session===null)return <div className="jp-login"><h1>Onward</h1>{failure?<button onClick={loadSession}>Retry · Réessayer · 重试</button>:<span className="jp-v1-orbit"/>}</div>;

...
export function OffersPage() {
  ...
  return <div className="jp-page onward-opportunities" data-testid="offers-page">
    ...
    <section className="onward-opportunity-search"><Input label={tr("搜索职位或公司","Rechercher un poste, une entreprise…","Search a role or company…")} value={query} onChange={e=>setQuery(e.target.value)}/><AiProgressButton taskKind="search" data-testid="search-offers" disabled={!query.trim()} onClick={()=>startTask({kind:"search",query:directions.find(d=>d.title===query)?.searchQuery || query,silent:true})}>{tr("搜索","Rechercher","Search")}</AiProgressButton><Hint>{data.v1?.searchNotice}</Hint>{!!offers.length&&<Button kind="text" data-testid="refresh-search" onClick={()=>startTask({kind:"search",query:String(discovery.query||directions.find(d=>d.title===query)?.searchQuery||query),silent:true,refresh:true})}>{tr("刷新当前结果","Actualiser ces résultats","Refresh these results")}</Button>}</section>
    ...
    {!offers.length&&<section className="jp-stack"><Empty title={data.v1?.searchIncomplete?tr("搜索暂未完成","La recherche est incomplète","Search is incomplete"):data.v1?.presentationFailed?tr("这次没有准备好，请再试一次","La sélection n’est pas encore prête","This selection needs another try"):data.v1?.backgroundActive?tr("为你挑选值得一试的工作","Une sélection qui vous correspond","Finding roles worth your time"):tr("这个方向暂时没有合适的岗位","Pas encore d’offre adaptée à cette piste","No suitable roles for this direction yet")}>{tr("换一个方向，或稍后再试。","Essayez une autre piste ou revenez un peu plus tard.","Try another direction or come back shortly.")}</Empty>{data.v1?.presentationFailed&&<Button onClick={()=>retryV1()}>{tr("重试","Réessayer","Retry")}</Button>}</section>}
    {!!offers.length&&data.v1?.searchProgress?.status==="failed"&&<Card><Hint>{tr("最新一次搜索没有完成；下面仍保留上一批可用结果。","La dernière recherche a échoué ; les résultats précédents restent disponibles ci-dessous.","The latest search did not finish; your previous usable results remain below.")}</Hint><Button kind="text" onClick={()=>retryV1()}>{tr("重试最新搜索","Réessayer la recherche","Retry latest search")}</Button></Card>}

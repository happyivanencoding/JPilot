...
import { AiProgressButton, Button, Card, Chip, Empty, Hint, Input, Pill, RowLink, Score, TextArea, Title } from "./ui";
...
function offerBrief(offer: Json) {
  ...
  duties = [...new Set([...duties, ...extractSection(description, ["responsibilities", "duties", "missions", "tasks", "mission", "responsabilités", "工作内容", "岗位职责"])])].slice(0, 3);
  requirements = [...new Set([...requirements, ...extractSection(description, ["requirements", "qualifications", "skills", "profile", "compétences", "profil", "岗位要求", "任职要求"])])].slice(0, 3);
  ...
  return { duties: duties.length ? duties : ["原始职位页未提供可提炼的工作内容。"], requirements: requirements.length ? requirements : ["原始职位页未提供可提炼的岗位要求。"] };
}

function openOfferSource(offer: Json) {
  if (typeof window === "undefined") return;
  const key = `jobpilot-source-${Date.now()}`;
  try { window.localStorage.setItem(key, JSON.stringify(offer)); } catch { /* Source page can still show its fallback state. */ }
...
function OfferCard({ offer, selected, onSelect }: { offer: Json; selected: boolean; onSelect: () => void }) {
  ...
  return <article className="jp-offer" data-testid="discovery-offer"><div className="jp-row jp-offer-select-row"><button type="button" className="jp-select-box" aria-pressed={selected} aria-label={selected ? tr("取消选择岗位", "Désélectionner l’offre", "Deselect role") : tr("选择岗位", "Sélectionner l’offre", "Select role")} onClick={onSelect}>{selected ? <CheckSquare size={22} /> : <Square size={22} />}</button><div className="jp-grow"><div className="jp-company">{offer.company}</div><h2>{offer.title}</h2></div></div><Hint>{[offer.location, offer.contractType !== "unknown" ? product(offer.contractType) : ""].filter(Boolean).join(" · ")}</Hint><Hint>{bits.join(" · ")}</Hint><p>{offer.why}</p><section className="jp-offer-brief"><div><h3>{tr("工作内容", "Missions", "What you will do")}</h3>{brief.duties.map((item, index) => <p className="jp-bullet" key={`duty-${index}`}>{item}</p>)}</div><div><h3>{tr("岗位要求", "Profil recherché", "What you bring")}</h3>{brief.requirements.map((item, index) => <p className="jp-bullet" key={`requirement-${index}`}>{item}</p>)}</div></section><Hint>{evaluating ? tr("分析中 · 不会重复启动", "Évaluation en cours · une seule tâche", "Evaluation in progress · one task only") : tr("待评估 · 岗位开放情况需核实", "À évaluer · disponibilité à confirmer", "Unassessed · availability unconfirmed")}</Hint><button type="button" className="jp-source-button" onClick={() => openOfferSource(offer)}>{tr("查看原信息", "Voir l’annonce originale", "View original posting")}<ArrowRight size={16} /></button><div className="jp-row"><Button kind="outline" disabled={Boolean(saved)} onClick={() => act({ action: "saveOffer", offer })}>{saved ? tr("已收藏", "Enregistrée", "Saved") : tr("保存", "Enregistrer", "Save")}</Button><AiProgressButton taskKind="evaluate" jobId={evaluateJobId} disabled={evaluating} onClick={() => startTask({ kind: "evaluate", url: offer.url, offer })}>{tr("岗位评估", "Évaluer", "Evaluate")}</AiProgressButton></div></article>;
}
export function OffersPage() {
  ...
  return <div className="jp-page" data-testid="offers-page"><Title sub={tr("已评估的岗位会收进「我的」，不会重复出现在这里。", "Les postes évalués se retrouvent dans « Moi ».", "Evaluated roles move into My space.")}>{tr("值得看的机会", "Les bonnes opportunités", "Worth a closer look")}</Title>
    ...
    {!offers.length && <Empty title={tr("这里没有待处理的岗位", "Aucune offre en attente ici", "No pending offers here")}>{tr("可以发起搜索，或到我的页面查看已有评估。", "Lancez une recherche ou consultez vos évaluations dans Moi.", "Search for opportunities or view your evaluations in My space.")}</Empty>}{offers.map(offer => <OfferCard key={offer.url} offer={offer} selected={selected.includes(String(offer.url))} onSelect={() => setSelected(previous=>previous.includes(String(offer.url))?previous.filter(url=>url!==String(offer.url)):[...previous,String(offer.url)])} />)}
  </div>;
}
export function ApplicationsPage() {
  ...
  const openCompare = () => navigate({ tab: "profile", filter, view: "compare", ids: chosen.join(",") });
  ...
  return <div className="jp-page" data-testid="applications-page" data-filter={filter} data-count={jobs.length}>
    ...
    <div className="jp-chips">{options.map(([key, label]) => <Chip key={key} selected={filter === key} data-testid={`filter-${key || "all"}`} onClick={() => navigate({ tab: "profile", view: "applications", filter: key }, true)}>{label}</Chip>)}</div>
    ...
    {["high","due","decide"].includes(filter)&&<Button kind="text" onClick={() => navigate({ tab: "profile", view: "applications" }, true)}>{tr("清除快捷筛选", "Effacer ce filtre", "Clear quick filter")}</Button>}

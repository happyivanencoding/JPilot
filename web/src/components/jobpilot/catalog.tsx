...
function OfferCard({offer}:{offer:Json}) {
  ...
  return <button type="button" className={`jp-v1-role-card jp-offer${offer.roleCv?" has-role-cv":""}`} data-testid="discovery-offer" onClick={()=>openOffer(String(offer.url))}>
    ...
    <Hint>{offer.deepMatchState==="loading"?tr("正在补充岗位匹配…","Match détaillé en cours…","Adding detailed match…"):deep.currentScore!=null?tr(`最初预估：${offer.matchScore?.baseline ?? deep.currentScore} → ${offer.matchScore?.forecast ?? deep.cvPotentialScore}/100`,`Estimation initiale : ${offer.matchScore?.baseline ?? deep.currentScore} → ${offer.matchScore?.forecast ?? deep.cvPotentialScore}/100`,`Initial estimate: ${offer.matchScore?.baseline ?? deep.currentScore} → ${offer.matchScore?.forecast ?? deep.cvPotentialScore}/100`):tr("点开看详细匹配","Ouvrez pour voir le match détaillé","Open for the detailed match")}</Hint>
    {offer.matchScore?.reviewed&&offer.matchScore.reviewedScore!=null&&<Hint>{tr(`本次复核：${offer.matchScore.baseline} → ${offer.matchScore.reviewedScore}`,`CV revu : ${offer.matchScore.baseline} → ${offer.matchScore.reviewedScore}`,`Reviewed CV: ${offer.matchScore.baseline} → ${offer.matchScore.reviewedScore}`)}{offer.matchScore.reviewedScore>offer.matchScore.baseline?` (+${offer.matchScore.reviewedScore-offer.matchScore.baseline})`:tr(" · 未提升"," · inchangé"," · unchanged")}</Hint>}
  </button>;
}

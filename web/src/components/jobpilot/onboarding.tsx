...
export function V1FirstRunOverlay({onDone}:{onDone:()=>void|Promise<void>}) {
  ...
  const login=()=>p.execute(async()=>{await p.request("/api/v1/session",{method:"POST",body:JSON.stringify({action:"login",email:email.trim()})});void fetch("/api/analytics",{method:"POST",credentials:"same-origin",keepalive:true,headers:{"Content-Type":"application/json"},body:JSON.stringify({events:[{id:crypto.randomUUID(),sessionId:crypto.randomUUID(),event:"page_exit",page:"onboarding_email",durationMs:Date.now()-enteredAt.current,scrollDepth:0},{id:crypto.randomUUID(),sessionId:crypto.randomUUID(),timestamp:Date.now(),event:"funnel",page:"onboarding_email",step:"login"}]})}).catch(()=>{});window.location.replace("/");});
  const pick=async(file:File)=>{setUploading(true);try{return await p.upload(file,sourceLanguage,locale,contracts,searchArea);}finally{setUploading(false);}};
  ...
  return <div className="jp-v1-first-run" data-testid="v1-first-run">{showPrivacy&&<CvPrivacyDialog onClose={()=>setShowPrivacy(false)} onAccepted={contracts.length&&areaValid?()=>picker.current?.click():undefined}/>}
    ...
    <div className="jp-v1-first-run-body" key={stage}>
      ...
      {stage===2&&<><h1>{tr("你的下一章，从这里开始。","Votre prochain chapitre commence ici.","Your next chapter starts here.")}</h1><Hint>{tr("上传简历，让 Onward 从你的真实经历出发找到与你匹配的机会。","Importez votre CV et laissez Onward trouver des opportunités en accord avec votre profil et vos ambitions.","Upload your CV and let Onward find opportunities aligned with your experience and ambitions.")}</Hint><strong>{tr("你想找哪类机会？","Quel type d’opportunité ?","What are you looking for?")}</strong><div className="jp-contract-grid">{["Stage","Alternance","CDI","CDD"].map(type=><button key={type} type="button" data-testid={`onboarding-contract-${type}`} className={`jp-chip ${contracts.includes(type)?"selected":""}`} aria-pressed={contracts.includes(type)} onClick={()=>setContracts(old=>old.includes(type)?old.filter(x=>x!==type):[...old,type])}>{p.product(type)}</button>)}</div>{!contracts.length&&<Hint>{tr("至少选择一项，可以多选。","Choisissez au moins une option, plusieurs sont possibles.","Select at least one. You can choose several.")}</Hint>}<SearchAreaFields value={searchArea} onChange={setSearchArea}/><details data-analytics="language_options"><summary>{tr("语言设置 · 自动","Langues · automatique","Languages · automatic")}</summary><Hint>{tr("自动识别简历语言；分析、岗位详情和公司信息跟随界面语言。","La langue du CV est détectée ; les analyses, les offres et les informations sur les entreprises suivent l’interface.","CV language is detected; analysis, role details and company information follow the interface language.")}</Hint><strong>{tr("求职简历语言","Langue du CV de candidature","Application CV language")}</strong><Button kind="text" onClick={()=>setSourceLanguage("auto")}>{tr("自动识别","Détection automatique","Auto-detect")}</Button>{languages(sourceLanguage,setSourceLanguage,true)}</details><input ref={picker} data-testid="onboarding-cv-input" type="file" hidden accept=".pdf,.docx,.txt,.md,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain,text/markdown" onChange={e=>{const input=e.currentTarget;const file=input.files?.[0];if(file)void pick(file).finally(()=>{input.value="";});}}/>{v.importState==="failed"&&<p role="alert">{tr("这份文件暂时打不开，请换一份 PDF 或 Word。","Ce fichier ne s’ouvre pas. Essayez un autre PDF ou Word.","This file could not be opened. Try another PDF or Word file.")}</p>}<div className="jp-v1-upload-card onward-upload-drop"><span className="onward-upload-icon"><FileText size={23}/></span><strong>{tr("把 CV 放在这里","Déposez votre CV ici","Drop your CV here")}</strong><Hint>{tr("或从你的文件中选择","ou choisissez-le dans vos fichiers","or choose it from your files")}</Hint><Button data-testid="onboarding-upload" disabled={busy||!contracts.length||!areaValid} onClick={()=>setShowPrivacy(true)}>{tr("选择文件","Choisir un fichier","Choose a file")}</Button><Hint>PDF · Word · TXT · 12 MB</Hint></div><button type="button" className="onward-data-note" data-testid="privacy-link" onClick={()=>setShowPrivacy(true)}><LockKeyhole size={18}/><span><strong>{tr("你的数据会受到保护","Vos données sont sécurisées","Your data is protected")}</strong><small>{tr("CV 只用于为当前档案提供匹配与建议。","Votre CV est utilisé uniquement pour les correspondances et conseils de ce profil.","Your CV is used only to provide matches and guidance for this profile.")}</small></span></button></>}
      ...
      {stage===5&&<><h1>{empty&&v.searchIncomplete?tr("搜索暂未完成","La recherche est incomplète","Search is incomplete"):empty?tr("这个方向暂时没有合适的岗位","Pas encore d’offre adaptée à cette piste","No suitable roles for this direction yet"):tr("为你挑选值得一试的工作","Une sélection qui vous correspond","Finding roles worth your time")}</h1>{!empty&&!waveFailed&&<><Hint>{progress.label||tr("正在找岗位","Recherche des offres","Finding roles")}</Hint><div className="jp-cv-progress" data-testid="first-search-progress" role="progressbar" aria-label={tr("岗位准备进度","Préparation des offres","Role preparation progress")} aria-valuemin={0} aria-valuemax={100} aria-valuenow={waterPercent}><strong>{progress.status==="completed"?"100%":`${waterPercent}%`}</strong><Hint>{tr("匹配、优势和建议，准备好后一起呈现。","Match, atouts et conseils arrivent ensemble.","Your match, strengths and insights arrive together.")}</Hint></div></>}{empty&&v.searchIncomplete&&<Hint>{v.searchNotice}</Hint>}{(waveFailed||empty&&v.searchIncomplete)&&<Button disabled={busy} onClick={()=>p.retryV1()}>{tr("再试一次","Réessayer","Try again")}</Button>}{(empty||waveFailed)&&<Button kind="text" onClick={()=>setChooseAgain(true)}>{tr("换个方向看看","Explorer une autre piste","Explore another direction")}</Button>}{empty&&<Button kind="text" onClick={()=>void onDone()}>{tr("先进入首页","Aller à l’accueil","Go to Home")}</Button>}</>}

...
export function V1FirstRunOverlay({onDone}:{onDone:()=>void|Promise<void>}) {
  ...
  const [languageChosen,setLanguageChosen]=useState(false),[invite,setInvite]=useState("V1TEST");
  ...
  const stage=!signedIn?(languageChosen?1:0):(loading||uploading||importing&&!p.error)?3:(v.importState==="failed"||!data.cv)?2:!v.analysisReady?3:(chooseAgain||!journey.query)?4:!v.offersReady?5:6;
  ...
  const login=()=>p.execute(async()=>{await p.request("/api/v1/session",{method:"POST",body:JSON.stringify({action:"login",invite})});window.location.replace("/");});
  ...
  return <div className="jp-v1-first-run" data-testid="v1-first-run">
    ...
    <div className="jp-v1-first-run-body" key={stage}>
      ...
      {stage===1&&<><h1>{tr("找到属于你的机会","Trouvez votre prochaine opportunité","Find your next opportunity")}</h1><label className="jp-field"><span>{tr("邀请码","Code d’invitation","Invitation code")}</span><input value={invite} onChange={e=>setInvite(e.target.value)} /></label><Hint>{tr("预览账号 · 邀请码 V1TEST","Compte d’aperçu · code V1TEST","Preview account · code V1TEST")}</Hint><Button data-testid="preview-login" disabled={busy||!invite.trim()} onClick={()=>void login()}>{tr("使用 Google 继续（模拟）","Continuer avec Google (simulation)","Continue with Google (preview)")}</Button><Button kind="text" onClick={()=>setLanguageChosen(false)}>{tr("更换语言","Changer de langue","Change language")}</Button></>}
      ...
      {stage===3&&<>{!v.presentationFailed&&!p.error&&<div className="jp-v1-orbit"/>}<h1>{tr("你的下一步，可以有哪些可能？","Quelles possibilités pour la suite ?","What could your next step look like?")}</h1><Hint>{tr("即将为你呈现优势、提升空间和可探索的方向。","Vos atouts, vos pistes de progrès et des directions à explorer.","Your strengths, room to grow and directions to explore.")}</Hint>{v.presentationFailed&&<Button disabled={busy} onClick={()=>p.retryV1()}>{tr("再试一次","Réessayer","Try again")}</Button>}</>}

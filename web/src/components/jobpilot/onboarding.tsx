...
export function V1FirstRunOverlay({onDone}:{onDone:()=>void|Promise<void>}) {
  ...
  const pick=async(file:File)=>{setUploading(true);await p.upload(file,sourceLanguage,analysisLanguage,contracts);setUploading(false);};
  ...
  return <div className="jp-v1-first-run" data-testid="v1-first-run">{showPrivacy&&<CvPrivacyDialog onClose={()=>setShowPrivacy(false)} onAccepted={contracts.length?()=>picker.current?.click():undefined}/>}
    ...
    <div className="jp-v1-first-run-body" key={stage}>
      ...
      {stage===2&&<><h1>{tr("从你的简历开始","Tout commence avec votre CV","It starts with your CV")}</h1><Hint>{tr("看看你擅长什么，以及哪些工作值得一试。","Découvrez vos atouts et les postes à explorer.","See what you bring and which roles are worth exploring.")}</Hint><strong>{tr("你想找哪类机会？","Quel type d’opportunité ?","What are you looking for?")}</strong><div className="jp-contract-grid">{["Stage","Alternance","CDI","CDD"].map(type=><button key={type} type="button" data-testid={`onboarding-contract-${type}`} className={`jp-chip ${contracts.includes(type)?"selected":""}`} aria-pressed={contracts.includes(type)} onClick={()=>setContracts(old=>old.includes(type)?old.filter(x=>x!==type):[...old,type])}>{p.product(type)}</button>)}</div>{!contracts.length&&<Hint>{tr("至少选择一项，可以多选。","Choisissez au moins une option, plusieurs sont possibles.","Select at least one. You can choose several.")}</Hint>}<strong>{tr("求职简历语言","Langue du CV de candidature","Application CV language")}</strong>{languages(sourceLanguage,setSourceLanguage,true)}<strong>{tr("我希望用这种语言看分析","Langue de mes conseils","My insights in")}</strong>{languages(analysisLanguage,value=>{setLanguageEdited(true);setAnalysisLanguage(value);})}<input ref={picker} data-testid="onboarding-cv-input" type="file" hidden accept=".pdf,.docx,.txt,.md" onChange={e=>{const file=e.target.files?.[0];e.target.value="";if(file)void pick(file);}}/>{v.importState==="failed"&&<p role="alert">{tr("这份文件暂时打不开，请换一份 PDF 或 Word。","Ce fichier ne s’ouvre pas. Essayez un autre PDF ou Word.","This file could not be opened. Try another PDF or Word file.")}</p>}<Button kind="text" data-testid="privacy-link" onClick={()=>setShowPrivacy(true)}>{tr("简历信息如何使用","Utilisation des informations du CV","How your CV information is used")}</Button><Button data-testid="onboarding-upload" disabled={busy||!contracts.length} onClick={()=>setShowPrivacy(true)}>{tr("选择简历","Choisir mon CV","Choose my CV")}</Button><Hint>PDF · Word · TXT · 12 MB</Hint></>}
...
function FirstRunOffer({offer,onClick}:{offer:Json;onClick:()=>void}) {
  ...
  return <button type="button" className="jp-v1-swipe-card" onClick={onClick}><div className="jp-row"><div className="jp-grow"><span className="jp-company">{offer.company}</span><h2>{offer.title}</h2></div><div className="jp-match100">{Number.isFinite(score)?Math.round(score):"—"}<span>/100</span></div></div><Hint>{[offer.location,offer.contractType!=="unknown"?product(offer.contractType):""].filter(Boolean).join(" · ")}</Hint><div className="jp-v1-signal-row">{strengths.slice(0,2).map((x,i)=><span className="jp-v1-plus" key={i}>+ {x}</span>)}</div><div className="jp-v1-signal-row">{gaps.slice(0,2).map((x,i)=><span className="jp-v1-minus" key={i}>− {x}</span>)}</div>{Number.isFinite(potential)&&potential>score&&<Hint>{tr(`简历优化：${Math.round(score)} → 预计 ${Math.round(potential)}/100`,`CV : ${Math.round(score)} → ~${Math.round(potential)}/100`,`CV edits: ${Math.round(score)} → ~${Math.round(potential)}/100`)}</Hint>}</button>;
}

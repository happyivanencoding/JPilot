...
export function V1FirstRunOverlay({onDone}:{onDone:()=>void|Promise<void>}) {
  ...
  const [sourceLanguage,setSourceLanguage]=useState("auto"),[analysisLanguage,setAnalysisLanguage]=useState<"en"|"fr"|"zh">(locale);
  ...
  const pick=async(file:File)=>{setUploading(true);await p.upload(file,sourceLanguage,analysisLanguage,contracts,searchArea);setUploading(false);};

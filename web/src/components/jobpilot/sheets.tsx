  ...
  return <Sheet title={<><div className="jp-subtitle">{job.company}</div><div className="jp-row"><span className="jp-grow">{job.role}</span>{job.v1Match?<Match100 value={job.v1Match.displayScore ?? job.v1Match.currentScore}/>:<Score score={job.score} />}</div></>} testId={`job-detail-${job.id}`}>
    ...
    <div className="jp-sheet-content" data-testid="job-content"><Localization value={job.localization} />
      {tab === 0 && <>
        ...
        {job.v1Match&&<Card><h3>{tr("简历匹配","Match de votre CV","Your CV match")}</h3><div className="jp-v1-potential"><div><span>{tr("当前匹配","Match actuel","Current match")}</span><strong>{Math.round(Number(job.v1Match.displayScore ?? job.v1Match.currentScore))}</strong></div><span>→</span><div><span>{tr("优化后预计","Après retouches, estimé","Estimated after edits")}</span><strong>~{Math.round(Number(job.v1Match.cvPotentialScore ?? job.v1Match.currentScore))}</strong></div></div></Card>}
...
function TailoredCvDraftPanel({ job }: { job: Json }) {
  ...
  return <section className="jp-card" data-testid="tailored-cv-draft"><div className="jp-row spread"><h3>{pending?tr("候选简历草稿","Brouillon de CV adapté","Tailored CV draft"):draft.status==='accepted'?tr("已保留的草稿记录","Brouillon conservé","Saved draft record"):tr("已拒绝的草稿","Brouillon refusé","Rejected draft")}</h3>{!job.v1Match&&<Pill warm={!draft.atsPass}>ATS {draft.atsScore ?? '—'}/100</Pill>}</div>
    {assessment.draftScore != null ? <div className="jp-cv-score-delta"><div><span>{tr("当前主简历","CV actuel","Current master CV")}</span><strong>{assessment.baselineScore}</strong></div><span className="jp-cv-score-arrow">→</span><div><span>{job.v1Match?tr("这个版本预计","Estimation pour ce CV","Estimated with this CV"):tr("这个草稿","Ce brouillon","This draft")}</span><strong>{assessment.draftScore}</strong></div><Pill warm={delta<0}>{delta>=0?'+':''}{delta}</Pill></div> : <Hint>{tr("这个草稿还没有完成岗位呈现评分。", "Ce brouillon n’a pas encore de score de présentation.", "This draft has not been presentation-scored yet.")}</Hint>}
    ...
    {!job.v1Match&&assessment.summary && <p>{assessment.summary}</p>}{texts(assessment.improvements).length>0&&<><strong>{tr("这次提升来自","D’où vient l’amélioration","What improved")}</strong>{texts(assessment.improvements).map((x,i)=><p className="jp-bullet" key={i}>{x}</p>)}</>}{texts(assessment.remainingGaps).length>0&&<><strong>{tr("仍然没有被简历解决","Ce que le CV ne résout pas","Still unresolved")}</strong>{texts(assessment.remainingGaps).map((x,i)=><p className="jp-bullet" key={i}>{x}</p>)}</>}

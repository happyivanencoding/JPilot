...
import {RoleCvJourney} from "./role-cv-journey";
...
export function JobSheet({ job,offer }: { job: Json;offer?:Json }) {
  ...
  const deepHasScore=deep.currentScore!=null&&Number.isFinite(Number(deep.currentScore));
  const deepState=String(job.enrichment?.deepMatchState || (Array.isArray(deep.quickBoosts)&&deepHasScore?'ready':'pending'));
  const deepReady=deepState==='ready'&&deepHasScore;
  const visibleScore=Number.isFinite(Number(job.matchScore?.current))?Number(job.matchScore.current):Number(deep.currentScore);
  ...
  const canGenerateRoleCv=cvDraft?.status!=="pending"&&(!cv.file||cv.inputVersionId!==data.cvState?.versionId);
    ...
    <div className="jp-sheet-content" data-testid="job-content" ref={content}>{job.localization?.failed&&<Localization value={job.localization} />}
      ...
      {tab===1&&<>
        ...
        {!v1AwaitingDeep&&<>
          ...
          {cvDraft && (cvDraft.status!=="accepted"||!job.v1Match) && <TailoredCvDraftPanel job={job} />}
...
function TailoredCvDraftPanel({ job }: { job: Json }) {
  ...
  return <section className="jp-card" data-testid="tailored-cv-draft">{!job.v1Match&&<div className="jp-row spread"><h3>{pending?tr("候选简历草稿","Brouillon de CV adapté","Tailored CV draft"):draft.status==='accepted'?tr("已保留的草稿记录","Brouillon conservé","Saved draft record"):tr("已拒绝的草稿","Brouillon refusé","Rejected draft")}</h3><Pill warm={!draft.atsPass}>ATS {draft.atsScore ?? '—'}/100</Pill></div>}
    ...
    {!job.v1Match&&assessment.summary && <p>{assessment.summary}</p>}{!job.v1Match&&texts(assessment.improvements).length>0&&<><strong>{tr("这次提升来自","D’où vient l’amélioration","What improved")}</strong>{texts(assessment.improvements).map((x,i)=><p className="jp-bullet" key={i}>{x}</p>)}</>}{!job.v1Match&&texts(assessment.remainingGaps).length>0&&<><strong>{tr("接下来可完善","Vos prochaines pistes","Next areas to develop")}</strong>{texts(assessment.remainingGaps).map((x,i)=><p className="jp-bullet" key={i}>{x}</p>)}</>}

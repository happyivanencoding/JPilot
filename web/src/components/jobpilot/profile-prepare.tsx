...
export function ProfilePage() {
  ...
  useEffect(()=>{setProfileTab(0);},[p.profileId]);
  ...
  return <div className="jp-page roomy" data-testid="profile-page">{privacyMode>0&&<CvPrivacyDialog onClose={()=>setPrivacyMode(0)} onAccepted={privacyMode===1?()=>picker.current?.click():undefined}/>}
    ...
    {!needsCv&&<Card><div className="jp-row spread"><h3>{tr("我的岗位版本","Mes versions par offre","My role-specific versions")}</h3><Hint>{jobs.length}</Hint></div><Hint>{tr("为不同岗位准备的简历，都在这里。","Retrouvez ici vos CV adaptés à chaque offre.","Your tailored CVs, organised by role.")}</Hint>{!jobs.length&&<Hint>{tr("当你在岗位页点击“查看我的 XX 分版本”，它会出现在这里。","Une offre apparaîtra ici lorsque vous demanderez votre version ciblée.","A role appears here after you request its tailored version.")}</Hint>}{jobs.map(job=><button type="button" className="jp-list-item" key={job.id} onClick={()=>openJob(job.id,job.cvDraft||job.cv?.file?1:0)}><div className="jp-row"><div className="jp-grow"><strong>{job.company}</strong><p>{job.role}</p></div>{Number.isFinite(Number(job.v1Match?.displayScore))&&<strong>{Math.round(Number(job.v1Match.displayScore))}/100</strong>}</div><Hint>{job.cvDraft?.status==="pending"?tr("候选 CV 等待你确认","Brouillon à confirmer","CV draft awaiting your decision"):job.cv?.file?tr("已有岗位版 CV","CV adapté conservé","Tailored CV saved"):tr("已保存岗位","Offre enregistrée","Role saved")}</Hint></button>)}</Card>}
    {!p.preview&&!needsCv&&directions.length>0&&<Card><h3>{tr("AI 建议的探索方向","Directions suggérées","Suggested directions")}</h3><Hint>{tr("这些是建议，不会覆盖你明确填写的目标。点“编辑”可以改成你自己的方向。","Ce sont des suggestions ; elles ne remplacent pas vos objectifs explicites.","These are suggestions and never override your explicit goals.")}</Hint><div className="jp-chips">{directions.map((d,i)=><Chip key={i} selected={false} onClick={()=>{setRoles(d.title);setEditing(true);}}>{d.title}</Chip>)}</div></Card>}
    <Card><h3>{tr("语言","Langues","Languages")}</h3><strong>{tr("界面与分析","Application et conseils","App and insights")}</strong><div className="jp-chips">{([["zh","中文"],["fr","Français"],["en","English"]] as const).map(([code,label])=><Chip key={code} selected={locale===code} disabled={busy} data-testid={`ui-language-${code}`} onClick={async()=>{if(!separateInsights)await p.changeAnalysisLanguage(code);setLocale(code);}}>{label}</Chip>)}</div><Button kind="text" disabled={busy} data-testid="separate-insights" onClick={()=>{setSeparateInsights(!separateInsights);if(separateInsights)void p.changeAnalysisLanguage(locale);}}>{tr("单独设置分析语言","Choisir une autre langue de conseils","Use a different insights language")}</Button>{separateInsights&&<><Hint>{tr("分析与建议用这种语言，菜单保持不变。","Les conseils dans cette langue, sans changer les menus.","Your insights in this language, without changing menus.")}</Hint><div className="jp-chips">{([["en","English"],["fr","Français"],["zh","中文"]] as const).map(([code,label])=><Chip key={code} selected={data.languageSettings?.analysisLanguage===code} disabled={busy} data-testid={`insight-language-${code}`} onClick={()=>p.changeAnalysisLanguage(code)}>{label}</Chip>)}</div></>}<hr/><strong>{tr("求职简历","CV de candidature","Application CV")}</strong><div className="jp-chips">{[["fr","Français"],["en","English"]].map(([code,label])=><Chip key={code} selected={material===code} disabled={busy} data-testid={`material-language-${code}`} onClick={()=>act({applicationLanguage:code},"/api/profile")}>{label}</Chip>)}</div><Hint>{tr("只决定新生成简历的语言，不限制岗位搜索。","La langue de vos prochains CV, pas un filtre sur les offres.","The language of new CVs, not a filter on job opportunities.")}</Hint></Card>
    ...
    <Card><h3>{tr("外观","Apparence","Appearance")}</h3><div className="jp-chips">{[["system",tr("系统","Système","System")],["light",tr("亮色","Clair","Light")],["dark",tr("暗色","Sombre","Dark")]].map(([key,label])=><Chip key={key} selected={theme===key} data-testid={`theme-${key}`} onClick={()=>setTheme(key)}>{label}</Chip>)}</div></Card>

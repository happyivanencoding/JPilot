...
export function ProfilePage() {
  ...
  const [metricSheet,setMetricSheet]=useState<0|1|2|3>(0);
  ...
  const appliedJobs=jobs.filter(hasApplied),roleCvJobs=jobs.filter(job=>job.cvDraft?.id||job.cv?.file);
  ...
  return <div className="jp-page roomy" data-testid="profile-page">{privacyMode>0&&<CvPrivacyDialog onClose={()=>setPrivacyMode(0)} onAccepted={privacyMode===1?()=>picker.current?.click():undefined}/>}
    {metricSheet>0&&<Sheet title={metricSheet===1?tr("投递跟踪","Mes candidatures","Applications"):metricSheet===2?tr("已保存岗位","Offres suivies","Saved roles"):tr("岗位版 CV","Versions de CV","CV versions")} onClose={()=>setMetricSheet(0)} testId="profile-metric-sheet"><div className="jp-sheet-content onward-profile-collection">{metricSheet===1?<ProfileApplications jobs={appliedJobs} heading={false}/>:metricSheet===2?collectionRows(jobs,0,tr("还没有保存岗位。","Aucune offre enregistrée.","No saved roles yet.")):collectionRows(roleCvJobs,1,tr("还没有岗位版 CV。","Aucun CV ciblé pour le moment.","No role-specific CV yet."))}</div></Sheet>}
    ...
    <div className="onward-profile-stats" aria-label={tr("职业档案统计","Résumé du profil","Profile summary")}><button type="button" data-testid="profile-metric-applications" onClick={()=>setMetricSheet(1)}><BriefcaseBusiness size={17}/><b>{appliedJobs.length}</b><small>{tr("投递 / 跟踪","Candidatures","Applications")}</small></button><button type="button" data-testid="profile-metric-saved" onClick={()=>setMetricSheet(2)}><Bookmark size={17}/><b>{jobs.length}</b><small>{tr("已保存岗位","Offres suivies","Saved roles")}</small></button><button type="button" data-testid="profile-metric-cv" onClick={()=>setMetricSheet(3)}><FileText size={17}/><b>{roleCvJobs.length}</b><small>{tr("岗位版 CV","Versions de CV","CV versions")}</small></button></div>
    <div className="onward-profile-menu">
    <ProfileSection testId="profile-section-cv" title={tr("我的简历","Mon CV","My CV")} summary={data.cvState?.cvVersion!=null?`CV ${data.cvState.cvVersion}`:tr("未上传","Non importé","Not uploaded")}><div className="jp-stack"><FileText size={30} className="jp-accent"/><Hint>{tr("PDF、Word、TXT · 最大 12 MB", "PDF, Word, TXT · 12 Mo maximum", "PDF, Word, TXT · up to 12 MB")}</Hint>

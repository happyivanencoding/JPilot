...
export function ProfilePage() {
  ...
  return <div className="jp-page roomy" data-testid="profile-page"><Title sub={needsCv?tr("先上传一份简历，JobPilot 才能建立你的个人档案。","Importez d’abord votre CV pour créer votre dossier personnel.","Upload a CV first so JobPilot can build your personal profile."):data.profile?.name}>{needsCv?tr("从你的简历开始","Commençons par votre CV","Start with your CV"):tr("我的", "Moi", "My")}</Title>
    ...
    <Card><h3>{tr("默认简历语言", "Langue par défaut du CV", "Default CV language")}</h3><Hint>{tr("新生成的求职材料使用这种语言。", "Pour vos prochains documents de candidature.", "For your next application documents.")}</Hint><div className="jp-chips">{[["fr", "Français"], ["en", "English"]].map(([key, label]) => <Chip key={key} selected={material === key} disabled={busy} data-testid={`material-language-${key}`} onClick={() => act({ applicationLanguage: key }, "/api/profile")}>{label}</Chip>)}</div></Card>

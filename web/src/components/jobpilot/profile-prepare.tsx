...
export function AnalysisEntry() {
  ...
  return <div className="jp-stack"><h3>{tr("让招聘方看到重点", "Les bons signaux, au premier regard", "The right signals, at first glance")}</h3><Hint>{tr("不是把每一段写满，而是让最有价值的证据占据正确的位置。", "Pas davantage de texte : une place juste pour vos preuves les plus utiles.", "Not more text: the right space for your strongest evidence.")}</Hint>
    ...
    {(!exists || a.stale) && <AiProgressButton taskKind="analysis" kind="outline" data-testid="update-analysis" disabled={running || !data.cv?.trim()} onClick={() => startTask({ kind: "analysis" })}>{a.stale ? tr("更新分析", "Mettre à jour l’analyse", "Update analysis") : tr("分析整份简历", "Analyser mon CV", "Analyze my CV")}</AiProgressButton>}
...
export function ProfilePage() {
  ...
  return <div className="jp-page roomy" data-testid="profile-page"><Title sub={needsCv?tr("先上传一份简历，JobPilot 才能建立你的个人档案。","Importez d’abord votre CV pour créer votre dossier personnel.","Upload a CV first so JobPilot can build your personal profile."):data.profile?.name}>{needsCv?tr("从你的简历开始","Commençons par votre CV","Start with your CV"):tr("我的档案", "Votre profil, vos preuves.", "Your profile. Your evidence.")}</Title>
    ...
    <Card><h3>{tr("连接", "Connexion", "Connection")}</h3><Hint>{tr("电脑、JobPilot 和 AgentDock 需要保持运行。", "Votre PC, JobPilot et AgentDock doivent rester allumés.", "Your PC, JobPilot and AgentDock must remain running.")}</Hint><Input label={tr("当前服务器", "Serveur actuel", "Current server")} readOnly value={origin} /><Button kind="outline" onClick={() => p.refresh()}>{tr("连接并刷新", "Connecter et actualiser", "Connect and refresh")}</Button>
      ...
      <Hint>{`JobPilot Web 0.5.2 · ${tr("与 Android 共享数据", "Données partagées avec Android", "Shared data with Android")}`}</Hint>
    </Card>
  </div>;
}

...
export function PdfPreview() {
  ...
  const cleanSource = route.job ? `/api/candidatures/cv?id=${encodeURIComponent(route.job)}${tailoredDraft ? `&draftId=${encodeURIComponent(draft)}` : ""}` : !meta ? "" : masterDraft && tab === 0 ? `/api/mobile/cv?versionId=${encodeURIComponent(meta.draft?.baseVersionId || "")}` : basePath;
  const source=cleanSource+(route.job && compareMode ? `&compare=${compareMode}` : "");
  useEffect(() => {
    if (!source) return;
  ...
  const compareControls=route.job ? <div className="jp-stack" data-testid="role-cv-compare"><Button kind="outline" disabled={!pdf} data-testid="compare-role-cv" onClick={()=>{setCompareMode(compareMode?"":"highlight");setZoom(1);}}>{compareMode?tr("关闭对比","Fermer la comparaison","Close comparison"):tr("对比原简历 · 高光改动","Comparer au CV original · surligner","Compare original · highlight changes")}</Button>{compareMode&&<><div className="jp-chips"><Button kind={compareMode==="highlight"?"primary":"outline"} onClick={()=>setCompareMode("highlight")}>{tr("高光改动","Modifications","Changes")}</Button><Button kind={compareMode==="baseline"?"primary":"outline"} onClick={()=>setCompareMode("baseline")}>{tr("原简历","CV original","Original CV")}</Button></div><Hint>{tr("绿色：新增文字 · 黄色：改写。不是新增经历。","Vert : texte ajouté · jaune : reformulé. Pas de nouvelles expériences.","Green: added wording · yellow: rewritten. Not new experience.")}</Hint></>}</div> : null;
  ...
  return <Sheet full title={tailoredDraft ? tr("检查岗位专属简历草稿", "Vérifier le brouillon adapté au poste", "Review tailored CV draft") : masterDraft ? tr("检查简历草稿", "Vérifier le brouillon", "Review draft") : tr("简历 PDF", "Votre CV · PDF", "Your CV · PDF")} testId="pdf-preview" footer={route.job ? <>{compareControls}{tailoredFooter}</> : (masterDraft ? <>

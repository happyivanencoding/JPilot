...
export function External({ url, children, icon=true }: { url: unknown; children: ReactNode; icon?:boolean }) { const safe = safeExternalUrl(url); return safe ? <a className="jp-external" href={safe} target="_blank" rel="noopener noreferrer">{children}{icon&&<ArrowUpRight size={16} />}</a> : null; }
...
export function Tabs({ labels, selected, onChange, prefix = "tab", muted=[] }: { labels: string[]; selected: number; onChange: (tab: number) => void; prefix?: string; muted?:number[] }) { return <div className="jp-tabs" role="tablist">{labels.map((label, i) => <button type="button" key={i} role="tab" aria-selected={selected === i} data-testid={`${prefix}-${i}`} className={muted.includes(i)?"jp-tab-locked":""} onClick={() => onChange(i)}>{muted.includes(i)&&<svg aria-hidden="true" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="5" y="10" width="14" height="11" rx="2"/><path d="M8 10V6a4 4 0 018 0v4"/></svg>}{label}</button>)}</div>; }
...
export function Sheet({ title, children, footer, full = false, compactHeader = false, onClose, testId, className = "" }: { title?: ReactNode; children: ReactNode; footer?: ReactNode; full?: boolean; compactHeader?: boolean; onClose?: () => void; testId?: string; className?: string }) {
  ...
  return <div className={`jp-overlay ${full ? "full" : ""} ${className}`.trim()} onClick={e => { if (e.target === e.currentTarget) dismiss(); }}>
    <div className="jp-sheet" role="dialog" aria-modal="true" aria-labelledby={id} ref={dialog} data-testid={testId} onKeyDown={e => {
      ...
      <header className={`jp-sheet-header${compactHeader ? " compact" : ""}`}><h2 id={id}>{title || tr("Onward 详情", "Détails Onward", "Onward details")}</h2><button className="jp-icon-button jp-close" type="button" aria-label={tr("关闭", "Fermer", "Close")} data-testid="close-sheet" onClick={dismiss}><X size={22} /></button></header>

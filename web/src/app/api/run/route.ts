...
export async function POST(req: Request) {
  let body: { kind?: string; input?: string; profileId?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const lang = readLanguageConfig(profileId);
  ...
  const acpMode = kind === "evaluate" || kind === "fix-portal" ? "agent" : "read-only";
  const acpModel = kind === "research" ? "gpt-5.6-terra" : "gpt-5.6-sol";
  const acpReasoning = kind === "research" ? "medium" : "medium";
  const spec = {
  ...
  const child = kind === "evaluate"
    ? startDetachedEvaluateWorker({ prompt, cwd: careerOpsRoot(), timeoutMs: killMsForKind(kind) })

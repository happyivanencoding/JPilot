...
export async function GET(req:Request) {
  try {
    ...
    return new Response(new Uint8Array(fs.readFileSync(rendered.pdf)),{headers:{"Content-Type":"application/pdf",...(url.searchParams.get("download")==="1"?{"Content-Disposition":"attachment; filename=Onward-CV.pdf"}:{}),"Cache-Control":"no-store","X-CV-Pages":String(rendered.pages)}});

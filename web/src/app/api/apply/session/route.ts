...
// Agentic fallbacks run through AgentDock ACP / Codex when deterministic
// extraction is low-confidence.
export async function POST(req: Request) {
  let body: { url?: string; agent?: boolean; _noApplyBtn?: boolean };
  try {
    body = await req.json();
  } catch {
  ...
  try {
    const session = await openSession(url, undefined, body.agent, body._noApplyBtn);

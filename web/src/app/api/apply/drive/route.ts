...
export async function POST(req: Request) {
  let body: { sessionId?: string; goal?: "reach" | "full"; answers?: { label: string; value: string }[] };
  try {
    body = await req.json();
  } catch {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        const result = await driveSession(page, goal, isFormReady, (step) => emit({ t: "step", ...step }), budget, answers);
        ...
        if (result.reached) {
          const fin = await finalizeDrivenSession(s.id);
          if (fin) {

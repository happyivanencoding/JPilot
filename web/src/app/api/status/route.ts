...
type CandidatureStore = { updatedAt?: string; jobs?: CandidatureJob[] };

function cockpitStatus(canon: string, job: CandidatureJob): string {
  if (canon === "Applied") return "Candidature envoyée";
  if (canon === "Responded") return "Réponse reçue";
  if (canon === "Interview") return String(job.status || "").includes("Entretien") ? String(job.status) : "Entretien RH";
  if (canon === "Offer") return "Offre reçue";
  if (canon === "Hired") return "Embauché";
  if (canon === "Rejected") return "Refus";
  if (canon === "Discarded" || canon === "SKIP") return "Écartée";
  return job.cv?.file ? "CV prêt" : "À candidater";
}

function updateCockpit(profileId: string, match: (job: CandidatureJob) => boolean, canon: string): boolean {
  const file = profileFile(profileId, "candidatures");
  try {
    const store = JSON.parse(fs.readFileSync(file, "utf8")) as CandidatureStore;
    const job = (Array.isArray(store.jobs) ? store.jobs : []).find(match);
    if (!job) return false;
    job.status = cockpitStatus(canon, job);
...
export async function POST(req: Request) {
  ...
  // Candidatures-only rows have no historical tracker number. Pipeline projects
  // them with a `c:<id>` key so the same status API can update the profile store
  // instead of maintaining a second client-side mutation path.
  if (typeof n === "string" && n.startsWith("c:")) {
    const id = n.slice(2).trim();
    if (!id) return NextResponse.json({ error: "candidature id required" }, { status: 400 });
    const profileId = await activeProfileId();
    if (!updateCockpit(profileId, (job) => job.id === id, canon)) {
      return NextResponse.json({ error: "candidature not found" }, { status: 404 });
    }
    return NextResponse.json({ ok: true, status: canon, changed: true, statusLogged: false });
  }
  ...
  // If this tracker row is also represented in the richer Candidatures cockpit,
  // keep the user-facing funnel state aligned after classic Pipeline/Home writes.
  const profileId = await activeProfileId();
  updateCockpit(profileId, (job) => String(job.reportNum ?? "") === row, canon);

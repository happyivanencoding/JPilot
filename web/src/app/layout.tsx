...
import { listProfiles } from "@/lib/profile-context";
import "./globals.css";

export const metadata: Metadata = {
  title: "career-ops — cockpit de candidatures",
  description: "Cockpit local de recherche d'emploi, candidatures, CV, entretiens et relances.",
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  const initialProfileId = await activeProfileId();
  const profiles = listProfiles().map(({ id, name, shortName, legacyUntagged }) => ({ id, name, shortName, legacyUntagged }));
  return (
    <html
      lang="fr"
    ...
    >
      <body className="font-sans antialiased">
        ...
        <AppShell initialProfileId={initialProfileId} profiles={profiles}>{children}</AppShell>
      </body>
    </html>
  );
}

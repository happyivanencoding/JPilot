...
function seed(id, index) {
  ...
  const job={id:'qa-role-1',company:'Sample Research',role:'Research Analyst',url:'https://example.test/jobs/research',location:'Paris',workMode:'Hybrid',contract:'CDI',score:4.6,reportNum:'901',status:'À candidater',stage:'preparing',evaluationState:'evaluated',lastChecked:'2026-09-08',summary:'Saved assessment',strengths:['Evidence from the test profile'],gaps:[{title:'Presentation practice',severity:'Modéré',why:'Test requirement',positioning:'Practice with examples'}],match:[{requirement:'SQL',fit:'Fort',evidence:'Synthetic evidence',action:'Prepare an example'}],followup:{nextAction:'Saved automatic next action',nextActionSource:'system',dueDate:'2026-09-07',note:''},replies:[],statusHistory:[],cv:{file:'fixture.pdf',atsScore:82,keywordCoverage:75,keywords:['SQL','Research'],changes:['Synthetic change'],inputVersionId:'qa-v1',pages:1,presentationScore:70,baselinePresentationScore:58,presentationDelta:12},cvDraft:{id:'qa-tailored-draft',status:'pending',baseVersionId:'qa-v1',language:'fr',notesLocale:'zh',revision:1,createdAt:'2026-09-08T10:00:00Z',updatedAt:'2026-09-08T10:00:00Z',payload:{summary:'Synthetic tailored summary',experience:[{company:'Sample Research',role:'Research Intern',location:'Paris',dates:'2026',bullets:['Documented research evidence']}],projects:[],education:[],skills:[{category:'IT',items:['SQL']}],change_notes:['Synthetic adaptation']},file:'output/qa-tailored.pdf',htmlFile:'output/qa-tailored.html',pages:1,atsScore:72,atsPass:false,atsGrade:'C',atsIssues:[{severity:'critical',message:'Synthetic ATS contact warning'}],keywordCoverage:null,changes:['Synthetic adaptation'],baselinePresentationScore:58,assessment:{baselineScore:58,draftScore:76,delta:18,summary:'Synthetic presentation improvement',improvements:['Relevant evidence appears earlier'],remainingGaps:['One real skill gap remains'],assessedAt:'2026-09-08T10:00:00Z',revision:1}},mobilePlan:{taskId:'qa-plan-saved',markdown:'Saved plan',questions:['Describe your research process.']},prepTasks:[{id:'prep-1',label:'Prepare an example',done:false}],interview:{processKnown:false,process:['Interview with team'],caseStudy:'Describe a research case.',questions:[{question:'Describe your research process.',answer:'Use a documented example.',proof:'Synthetic project'}]}};
  ...
  async function handler(route) {
    ...
    if(u.pathname==='/api/mobile'&&req.method()==='POST'){
      if(body.action==='updateTailoredCvDraft'){
        const job=f.jobs.find(j=>j.cvDraft?.id===body.draftId);if(!job)return send({error:'Draft missing'},404);job.cvDraft.payload=structuredClone(body.payload);job.cvDraft.revision++;job.cvDraft.assessment=null;job.cvDraft.updatedAt='2026-09-08T12:01:00Z';return send({ok:true,draft:job.cvDraft});
      }
      if(body.action==='decideTailoredCvDraft'){
        const job=f.jobs.find(j=>j.cvDraft?.id===body.draftId);if(!job)return send({error:'Draft missing'},404);job.cvDraft.status=body.decision==='accept'?'accepted':'rejected';if(body.decision==='accept')job.cv={...job.cv,file:job.cvDraft.file,atsScore:job.cvDraft.atsScore,presentationScore:job.cvDraft.assessment?.draftScore??null,baselinePresentationScore:job.cvDraft.baselinePresentationScore,presentationDelta:job.cvDraft.assessment?.delta??null,draftId:job.cvDraft.id};return send({ok:true,job,draft:job.cvDraft});
      }
      ...
      if(body.action==='batchTasks'){
        const tasks=(body.inputs||[]).map((input,index)=>{
          let jobId=input.jobId;
          if(input.kind==='evaluate'&&input.offer){let job=f.jobs.find(j=>j.url===input.url);if(!job){job={id:`qa-auto-saved-${index+1}`,company:input.offer.company,role:input.offer.title,url:input.url,status:'À candidater',stage:'preparing',score:null,evaluationState:'saved',followup:{nextAction:'Evaluate',dueDate:'',note:''}};f.jobs.push(job);}const discovery=f.discovery.offers.find(o=>o.url===input.url);if(discovery)discovery.jobId=job.id;jobId=job.id;}
          const task={id:`qa-task-${++serial}`,kind:input.kind,input,status:'queued',title:'Synthetic background task',createdAt:new Date().toISOString(),inputVersionId:f.cvState.versionId,jobId,estimate:{label:'预计耗时 10–30 秒',minSeconds:10,maxSeconds:30,targetSeconds:30,source:'verified-baseline'},metrics:{},result:{},destination:{jobId}};f.tasks.unshift(task);return task;
        });
      ...
      if(body.action==='task'){
        const input=body.input,kind=input.kind,qaAsync=String(input.question||'').startsWith('__QA_ASYNC__'),task={id:`qa-task-${++serial}`,kind,input,status:kind==='search'||qaAsync?'queued':'completed',title:'Synthetic test task',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),inputVersionId:f.cvState.versionId,jobId:input.jobId,estimate:{label:'预计耗时 10–30 秒',minSeconds:10,maxSeconds:30,targetSeconds:30,source:'verified-baseline'},metrics:{wallMs:2100,totalTokens:500,estimatedCostUsd:.001},result:{markdown:pick(locale,'# 已保存的测试反馈\n\n清晰的行动建议。','# Retour de test enregistré\n\nProchaine action claire.','# Saved test feedback\n\nA clear next step.')},destination:{jobId:input.jobId}};
        ...
        if(kind==='evaluate'){let job=f.jobs.find(j=>j.url===input.url);if(!job&&input.offer){job={id:`qa-auto-saved-${serial}`,company:input.offer.company,role:input.offer.title,url:input.url,status:'À candidater',stage:'preparing',score:null,evaluationState:'saved',followup:{nextAction:'Evaluate',dueDate:'',note:''}};f.jobs.push(job);const discovery=f.discovery.offers.find(o=>o.url===input.url);if(discovery)discovery.jobId=job.id;}job ||= f.jobs[0];job.evaluationState='evaluated';job.score=4.6;job.summary='Saved evaluation';task.jobId=job.id;task.destination.jobId=job.id;task.result.jobId=job.id;}
        if(kind==='cv_review'){const job=f.jobs.find(j=>j.id===input.jobId);job.cvDraft.assessment={baselineScore:job.cvDraft.baselinePresentationScore,draftScore:80,delta:22,summary:'Synthetic reassessment after manual edit',improvements:['Edited summary is more relevant'],remainingGaps:['One real skill gap remains'],assessedAt:'2026-09-08T12:02:00Z',revision:job.cvDraft.revision};task.jobId=job.id;task.destination.jobId=job.id;task.result={jobId:job.id,draftId:job.cvDraft.id,assessment:job.cvDraft.assessment};}

...
test("no CLI configured -> auth hint", () => {
  ...
  assert.equal(hint?.text, "Vérifiez qu'AgentDock et Codex sont connectés, puis relancez.");
});
...
test("connection error -> connection hint, NOT auth", () => {
  ...
  assert.equal(hint?.text, "Connexion au serveur local perdue — relancez.");
});
...
test("page-reload interruption -> interrupted hint, NOT auth", () => {
  ...
  assert.equal(hint?.text, "L'exécution a été interrompue — relancez.");
});

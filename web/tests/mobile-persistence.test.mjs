...
const {findExistingCandidature}=await import('../src/app/api/candidatures/route.ts');
...
test('evaluating a discovery offer saves it first even when the exact completed evaluation is reused',async()=>{
 ...
 const saved=engine.readCandidatureStore(q).jobs.find(job=>job.url===url);
...
test('tailored CV remains a draft until explicit keep; reject never replaces the saved tailored CV',async()=>{
 ...
 const before=JSON.stringify(engine.readCandidatureStore(p).jobs[0].cv);
 ...
 const rejectedBefore=JSON.stringify(engine.readCandidatureStore(p).jobs.find(j=>j.id==='draft-reject').cv);
 await decideTailoredCvDraft(p,'draft-no','reject');const rejected=engine.readCandidatureStore(p).jobs.find(j=>j.id==='draft-reject');assert.equal(JSON.stringify(rejected.cv),rejectedBefore);assert.equal(rejected.cvDraft.status,'rejected');

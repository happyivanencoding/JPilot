...
test('tailored CV remains a draft until explicit keep; reject never replaces the saved tailored CV',async()=>{
 const file=path.join(root,profiles[0].candidatures);
 const basis={currentScore:55,cvPotentialScore:68,capabilityPotentialScore:76,deepMatch:{currentScore:55,cvPotentialScore:68,capabilityPotentialScore:76,quickBoosts:[],scoringVersion:'role-fit-2'}};
 const baseJob={id:'draft-job',company:'Fixture',role:'Analyst',url:'https://example.org/draft-job',status:'À candidater',score:3,v1Match:structuredClone(basis),cv:{file:'output/original.pdf',presentationScore:55},prepTasks:[],followup:{nextAction:'',dueDate:'',note:''}};
 const draft={id:'draft-keep',status:'pending',baseVersionId:'fixture-version',language:'en',notesLocale:'zh',revision:1,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),payload:{summary:'draft',experience:[],projects:[],education:[],skills:[]},file:'output/draft.pdf',htmlFile:'output/draft.html',pages:1,atsScore:82,atsPass:true,atsIssues:[],keywordCoverage:75,changes:['Focused evidence'],baselinePresentationScore:55,matchBasis:structuredClone(basis),assessment:{baselineScore:55,draftScore:74,delta:19,summary:'better presentation',improvements:[],remainingGaps:[],assessedAt:new Date().toISOString(),revision:1}};
 ...
 const kept=await decideTailoredCvDraft(p,'draft-keep','accept');assert.equal(kept.job.cv.file,'output/draft.pdf');assert.equal(kept.job.cv.presentationScore,74);assert.equal(kept.job.v1Match.displayScore,68);assert.equal(kept.job.v1Match.acceptedCvScore,68);assert.equal(kept.job.status,'CV prêt');
